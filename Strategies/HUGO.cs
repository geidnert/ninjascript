...
namespace NinjaTrader.NinjaScript.Strategies.AutoEdge
{
    public class HUGO : Strategy
    {
        ...
        // same as HUGO-2): heartbeat reporter (TopstepX) + ProjectX router.
...
2026-12-31,08:30";
        ...
        protected override void OnStateChange()
        {
            if (State == State.SetDefaults)
            {
                ...
                GlobalMaPeriod=37; GlobalEmaPeriod=90; GlobalMaType=HugoMAMode_EMA3790.Both;
                // ── Global Contracts ─────────────────────────────────────────
                ContractQuantity=1;
                ...
                // ═══════════════════════════════════════════════════════════
                // ── Commercial / AutoEdge defaults (mirror HUGO-2) ────
                // ═══════════════════════════════════════════════════════════
                ...
                // Global daily risk (points). Default OFF so HUGO reproduces
                // the MichalEMA base exactly; enable per-user if desired. (Threshold
                // values retained from HUGO-2 for convenience.)
        ...
        private int  S_Contracts(int sid) { return ContractQuantity; }
        ...
        private HugoMAMode_EMA3790 SD_MaType(int sid,int d)            { return GlobalMaType; }
        ...
        private HugoBEMode2_EMA3790 SD_BreakevenMode(int sid,int d)    { return HugoBEMode2_EMA3790.FixedTicks; }
        ...
        private HugoEntryMode_EMA3790 SD_EntryMode(int sid,int d) { return HugoEntryMode_EMA3790.Market; } // V1.00: Market only for now, per-session override in v1.01
        ...
        private HugoTPMode_EMA3790 SD_TakeProfitMode(int sid,int d) { return HugoTPMode_EMA3790.CandleMultiple; } // V1.00: CandleMultiple as V106 default
        ...
        private double GetUpperMA(int sid, int dir, int barsAgo)
        {
            ...
            {
                case HugoMAMode_EMA3790.SMA:  return sh[barsAgo];
                case HugoMAMode_EMA3790.EMA:  return eh[barsAgo];
                case HugoMAMode_EMA3790.Both: return Math.Max(sh[barsAgo], eh[barsAgo]);
        ...
        private double GetLowerMA(int sid, int dir, int barsAgo)
        {
            ...
            switch (SD_MaType(sid,dir))
            {
                case HugoMAMode_EMA3790.SMA:  return sl[barsAgo];
                case HugoMAMode_EMA3790.EMA:  return el[barsAgo];
                case HugoMAMode_EMA3790.Both: return Math.Min(sl[barsAgo], el[barsAgo]);
        ...
        private double GetLowerMAForTrail(int sid, int dir)
        {
            ...
            switch (SD_MaType(sid,dir))
            {
                case HugoMAMode_EMA3790.SMA:  return sl[0];
                case HugoMAMode_EMA3790.EMA:  return el[0];
                case HugoMAMode_EMA3790.Both: return Math.Max(sl[0], el[0]);
                default:          return sl[0];
            }
        }
        private double GetUpperMAForTrail(int sid, int dir)
        {
            ...
            switch (SD_MaType(sid,dir))
            {
                case HugoMAMode_EMA3790.SMA:  return sh[0];
                case HugoMAMode_EMA3790.EMA:  return eh[0];
                case HugoMAMode_EMA3790.Both: return Math.Min(sh[0], eh[0]);
                default:          return sh[0];
            }
        }
        private bool IsSlopeRising(int sid, int dir, int lb)
        {
            ...
            switch(SD_MaType(sid,dir)){ case HugoMAMode_EMA3790.SMA:return s; case HugoMAMode_EMA3790.EMA:return e; case HugoMAMode_EMA3790.Both:return s&&e; default:return s; }
        }
        ...
        private void SubmitEntry(int sid, int dir, double close, double cH, double cL, double pH, double pL)
        {
            ...
            string name = dir==1 ? LongEntrySignalName : ShortEntrySignalName;
            HugoEntryMode_EMA3790 mode = SD_EntryMode(sid,dir);
            if (mode == HugoEntryMode_EMA3790.LimitOffset)
            {
            ...
            else if (mode == HugoEntryMode_EMA3790.LimitRetracement)
            {
            ...
            int qty, MarketPosition mp, string orderId, DateTime time)
        {
            ...
            if ((n==LongEntrySignalName||n==ShortEntrySignalName) && exec.Order.OrderState==OrderState.Filled && !hasActivePosition)
            {
                ...
                string eName = d==1?LongEntrySignalName:ShortEntrySignalName;
        ...
        private double CalculateTakeProfit(int sid, int dir, double fill)
        {
            ...
            {
                case HugoTPMode_EMA3790.FixedTicks:     dist = SD_TakeProfitTicks(sid,dir)*TickSize; break;
                case HugoTPMode_EMA3790.SwingPoint:     return GetSwingTarget(sid,fill,dir);
                case HugoTPMode_EMA3790.CandleMultiple: dist = signalCandleRange * SD_CandleMultiplier(sid,dir); break;
        ...
        private double CalculateTPDistanceTicks(int sid, int dir, double range)
        {
            ...
            switch (SD_TakeProfitMode(sid,dir))
            {
                case HugoTPMode_EMA3790.CandleMultiple: t = (range * SD_CandleMultiplier(sid,dir))/TickSize; break;
        ...
        private void ManageBreakeven(int sid)
        {
            ...
            double trig = SD_BreakevenMode(sid,d)==HugoBEMode2_EMA3790.FixedTicks
        ...
        private void CheckOpposingBarExit(int sid)
        {
            ...
            if (d==-1&&Position.MarketPosition==MarketPosition.Short&&bull)
            {
                ...
                else if (Close[0]>opposingBarBenchmark) ExitShort(Position.Quantity,"OppBarExit",ShortEntrySignalName);
            }
            if (d== 1&&Position.MarketPosition==MarketPosition.Long&&bear)
            {
                if (!opposingBarBenchmarkSet) { opposingBarBenchmark=Close[0]; opposingBarBenchmarkSet=true; }
                else if (Close[0]<opposingBarBenchmark) ExitLong(Position.Quantity,"OppBarExit",LongEntrySignalName);
            }
        }
        ...
        {
            CancelWorkingEntryOrder();
            if (Position.MarketPosition==MarketPosition.Long)  ExitLong(Position.Quantity,"ForcedExit",LongEntrySignalName);
            if (Position.MarketPosition==MarketPosition.Short) ExitShort(Position.Quantity,"ForcedExit",ShortEntrySignalName);
        }
        ...
        // ── Global ─────────────────────────────────────────────────────────────
        [NinjaScriptProperty][PropertyEditor("NinjaTrader.Gui.Tools.TimeEditorKey")][Browsable(false)][Display(Name="Session Start Time (Daily Reset Anchor)",Description="CME daily reset anchor - typically 18:00 for NQ.",Order=1,GroupName="0. Common - Time Settings")] public DateTime SessionStartTime {get;set;}
        [NinjaScriptProperty][Display(Name="NY Enable (Parent)",Order=2,GroupName="0. Global")] [Browsable(false)] public bool NyEnable {get;set;}
        [NinjaScriptProperty][Display(Name="EU Enable (Parent)",Order=3,GroupName="0. Global")] [Browsable(false)] public bool EuEnable {get;set;}
        [NinjaScriptProperty][Display(Name="AS Enable (Parent)",Order=4,GroupName="0. Global")] [Browsable(false)] public bool AsEnable {get;set;}
        ...
        [NinjaScriptProperty][Range(1,500)][Display(Name="EMA Fast Period (was MA)",  Order=10,GroupName="0. Global")] [Browsable(false)] public int    GlobalMaPeriod  {get;set;}
        [NinjaScriptProperty][Range(1,500)][Display(Name="EMA Slow Period", Order=11,GroupName="0. Global")] [Browsable(false)] public int    GlobalEmaPeriod {get;set;}
        [NinjaScriptProperty][Display(Name="MA Type (keep Both)",              Order=12,GroupName="0. Global")] [Browsable(false)] public HugoMAMode_EMA3790 GlobalMaType   {get;set;}
        // ── Global Contracts ─────────────────────────────────────────────────
        [NinjaScriptProperty][Range(1,int.MaxValue)][Display(Name="Contract Quantity",Order=2,GroupName="0. Common - Time Settings")] public int ContractQuantity {get;set;}
        // ── Global PriceOffset Trail ──────────────────────────────────────────
        [NinjaScriptProperty][Display(Name="Enable PriceOffset Trail (Global)",Order=14,GroupName="0. Global")] [Browsable(false)] public bool GlobalEnablePriceOffsetTrail {get;set;}



        // ── NY-A Session ────────────────────────────────────────────────────────
        [NinjaScriptProperty][Display(Name="Enable",Order=1,GroupName="1a. NY-A Session")] [Browsable(false)] public bool NyAEnable {get;set;}
        [NinjaScriptProperty][PropertyEditor("NinjaTrader.Gui.Tools.TimeEditorKey")][Display(Name="Trade Window Start",Order=2,GroupName="1a. NY-A Session")] [Browsable(false)] public DateTime NyATradeWindowStart {get;set;}
        [NinjaScriptProperty][Display(Name="Enable No-New-Trades After",Order=3,GroupName="1a. NY-A Session")] [Browsable(false)] public bool NyAEnableNoNewTradesAfter {get;set;}
        [NinjaScriptProperty][PropertyEditor("NinjaTrader.Gui.Tools.TimeEditorKey")][Display(Name="No New Trades After",Order=4,GroupName="1a. NY-A Session")] [Browsable(false)] public DateTime NyANoNewTradesAfter {get;set;}
        [NinjaScriptProperty][Display(Name="Enable Forced Close",Order=5,GroupName="1a. NY-A Session")] [Browsable(false)] public bool NyAEnableForcedClose {get;set;}
        [NinjaScriptProperty][PropertyEditor("NinjaTrader.Gui.Tools.TimeEditorKey")][Display(Name="Forced Close Time",Order=6,GroupName="1a. NY-A Session")] [Browsable(false)] public DateTime NyAForcedCloseTime {get;set;}
        [NinjaScriptProperty][Range(1,9999)][Display(Name="Max Profit Ticks",Order=10,GroupName="1a. NY-A Session")] [Browsable(false)] public int NyAMaxSessionProfitTicks {get;set;}
        [NinjaScriptProperty][Range(1,9999)][Display(Name="Max Loss Ticks",Order=11,GroupName="1a. NY-A Session")] [Browsable(false)] public int NyAMaxSessionLossTicks {get;set;}
                [NinjaScriptProperty][Range(1,20)][Display(Name="Max Trades",Order=8,GroupName="1a. NY-A Session")] [Browsable(false)] public int NyAMaxTradesPerSession {get;set;}
        [NinjaScriptProperty][Range(0,20)][Display(Name="Max Losses",Order=9,GroupName="1a. NY-A Session")] [Browsable(false)] public int NyAMaxLossesPerSession {get;set;}
        [NinjaScriptProperty][Display(Name="Require Direction Flip",Order=15,GroupName="1a. NY-A Session")] [Browsable(false)] public bool NyARequireDirectionFlip {get;set;}
        [NinjaScriptProperty][Display(Name="Allow Same Dir After Loss",Order=16,GroupName="1a. NY-A Session")] [Browsable(false)] public bool NyAAllowSameDirectionAfterLoss {get;set;}
        [NinjaScriptProperty][Range(0,200)][Display(Name="Engulfing Exit After Bars",Order=17,GroupName="1a. NY-A Session")] [Browsable(false)] public int NyAEngulfingExitAfterBars {get;set;}
        [NinjaScriptProperty][Range(0,50)][Display(Name="Engulfing Exit Padding Ticks",Order=18,GroupName="1a. NY-A Session")] [Browsable(false)] public int NyAEngulfingExitPaddingTicks {get;set;}
        // NY-A Long
        [NinjaScriptProperty][Range(0.01,100)][Display(Name="Candle Multiplier",Order=4,GroupName="1b. NY-A Long")] [Browsable(false)] public double NyALongCandleMultiplier {get;set;}
        [NinjaScriptProperty][Range(0,9999)][Display(Name="Max TP Ticks",Order=5,GroupName="1b. NY-A Long")] [Browsable(false)] public int NyALongMaxTakeProfitTicks {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="SL Extra Ticks",Order=6,GroupName="1b. NY-A Long")] [Browsable(false)] public int NyALongSlExtraTicks {get;set;}
        [NinjaScriptProperty][Range(0,9999)][Display(Name="Max SL Ticks",Order=7,GroupName="1b. NY-A Long")] [Browsable(false)] public int NyALongMaxSlTicks {get;set;}
        [NinjaScriptProperty][Range(0,10)][Display(Name="Max SL:TP Ratio",Order=8,GroupName="1b. NY-A Long")] [Browsable(false)] public double NyALongMaxSlToTpRatio {get;set;}
        [NinjaScriptProperty][Display(Name="Use Prior Candle SL",Order=9,GroupName="1b. NY-A Long")] [Browsable(false)] public bool NyALongUsePriorCandleSl {get;set;}
        [NinjaScriptProperty][Display(Name="SL At MA",Order=10,GroupName="1b. NY-A Long")] [Browsable(false)] public bool NyALongSlAtMa {get;set;}
        [NinjaScriptProperty][Display(Name="Move SL To Entry Bar",Order=11,GroupName="1b. NY-A Long")] [Browsable(false)] public bool NyALongMoveSlToEntryBar {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="Trail Offset Ticks",Order=12,GroupName="1b. NY-A Long")] [Browsable(false)] public int NyALongTrailOffsetTicks {get;set;}
        [NinjaScriptProperty][Range(0,100)][Display(Name="Trail Delay Bars",Order=13,GroupName="1b. NY-A Long")] [Browsable(false)] public int NyALongTrailDelayBars {get;set;}
        [NinjaScriptProperty][Range(0,500)][Display(Name="Trail Candle Offset",Order=14,GroupName="1b. NY-A Long")] [Browsable(false)] public int NyALongTrailCandleOffset {get;set;}
        [NinjaScriptProperty][Display(Name="Enable Breakeven",Order=15,GroupName="1b. NY-A Long")] [Browsable(false)] public bool NyALongEnableBreakeven {get;set;}
        [NinjaScriptProperty][Range(1,9999)][Display(Name="BE Trigger Ticks",Order=17,GroupName="1b. NY-A Long")] [Browsable(false)] public int NyALongBreakevenTriggerTicks {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="BE Offset Ticks",Order=19,GroupName="1b. NY-A Long")] [Browsable(false)] public int NyALongBreakevenOffsetTicks {get;set;}
        [NinjaScriptProperty][Range(0,200)][Display(Name="Max Bars In Trade",Order=20,GroupName="1b. NY-A Long")] [Browsable(false)] public int NyALongMaxBarsInTrade {get;set;}
        [NinjaScriptProperty][Display(Name="Enable PriceOffset Trail",Order=21,GroupName="1b. NY-A Long")] [Browsable(false)] public bool NyALongEnablePriceOffsetTrail {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="PriceOffset Reduction",Order=22,GroupName="1b. NY-A Long")] [Browsable(false)] public int NyALongPriceOffsetReductionTicks {get;set;}
        [NinjaScriptProperty][Display(Name="Enable WMA Filter",Order=29,GroupName="1b. NY-A Long")] [Browsable(false)] public bool NyALongEnableWmaFilter {get;set;}
        [NinjaScriptProperty][Range(1,500)][Display(Name="WMA Period",Order=30,GroupName="1b. NY-A Long")] [Browsable(false)] public int NyALongWmaPeriod {get;set;}
        [NinjaScriptProperty][Range(0,100)][Display(Name="Min Body %",Order=32,GroupName="1b. NY-A Long")] [Browsable(false)] public double NyALongMinBodyPct {get;set;}
        [NinjaScriptProperty][Range(0,200)][Display(Name="Trend Confirm Bars",Order=33,GroupName="1b. NY-A Long")] [Browsable(false)] public int NyALongTrendConfirmBars {get;set;}
        [NinjaScriptProperty][Display(Name="Enable ADX Filter",Order=35,GroupName="1b. NY-A Long")] [Browsable(false)] public bool NyALongEnableAdxFilter {get;set;}
        [NinjaScriptProperty][Range(1,100)][Display(Name="ADX Period",Order=36,GroupName="1b. NY-A Long")] [Browsable(false)] public int NyALongAdxPeriod {get;set;}
        [NinjaScriptProperty][Range(0.0,100.0)][Display(Name="ADX Min Level",Order=37,GroupName="1b. NY-A Long")] [Browsable(false)] public double NyALongAdxMinLevel {get;set;}
        [NinjaScriptProperty][Range(0.0,100.0)][Display(Name="ADX Max Level",Order=38,GroupName="1b. NY-A Long")] [Browsable(false)] public double NyALongAdxMaxLevel {get;set;}
        [NinjaScriptProperty][Range(0.0,100.0)][Display(Name="ADX Peak Drawdown",Order=39,GroupName="1b. NY-A Long")] [Browsable(false)] public double NyALongAdxPeakDrawdown {get;set;}
        // NY-A Short
        [NinjaScriptProperty][Range(0.01,100)][Display(Name="Candle Multiplier",Order=4,GroupName="1c. NY-A Short")] [Browsable(false)] public double NyAShortCandleMultiplier {get;set;}
        [NinjaScriptProperty][Range(0,9999)][Display(Name="Max TP Ticks",Order=5,GroupName="1c. NY-A Short")] [Browsable(false)] public int NyAShortMaxTakeProfitTicks {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="SL Extra Ticks",Order=6,GroupName="1c. NY-A Short")] [Browsable(false)] public int NyAShortSlExtraTicks {get;set;}
        [NinjaScriptProperty][Range(0,9999)][Display(Name="Max SL Ticks",Order=7,GroupName="1c. NY-A Short")] [Browsable(false)] public int NyAShortMaxSlTicks {get;set;}
        [NinjaScriptProperty][Range(0,10)][Display(Name="Max SL:TP Ratio",Order=8,GroupName="1c. NY-A Short")] [Browsable(false)] public double NyAShortMaxSlToTpRatio {get;set;}
        [NinjaScriptProperty][Display(Name="Use Prior Candle SL",Order=9,GroupName="1c. NY-A Short")] [Browsable(false)] public bool NyAShortUsePriorCandleSl {get;set;}
        [NinjaScriptProperty][Display(Name="SL At MA",Order=10,GroupName="1c. NY-A Short")] [Browsable(false)] public bool NyAShortSlAtMa {get;set;}
        [NinjaScriptProperty][Display(Name="Move SL To Entry Bar",Order=11,GroupName="1c. NY-A Short")] [Browsable(false)] public bool NyAShortMoveSlToEntryBar {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="Trail Offset Ticks",Order=12,GroupName="1c. NY-A Short")] [Browsable(false)] public int NyAShortTrailOffsetTicks {get;set;}
        [NinjaScriptProperty][Range(0,100)][Display(Name="Trail Delay Bars",Order=13,GroupName="1c. NY-A Short")] [Browsable(false)] public int NyAShortTrailDelayBars {get;set;}
        [NinjaScriptProperty][Range(0,500)][Display(Name="Trail Candle Offset",Order=14,GroupName="1c. NY-A Short")] [Browsable(false)] public int NyAShortTrailCandleOffset {get;set;}
        [NinjaScriptProperty][Display(Name="Enable Breakeven",Order=15,GroupName="1c. NY-A Short")] [Browsable(false)] public bool NyAShortEnableBreakeven {get;set;}
        [NinjaScriptProperty][Range(1,9999)][Display(Name="BE Trigger Ticks",Order=17,GroupName="1c. NY-A Short")] [Browsable(false)] public int NyAShortBreakevenTriggerTicks {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="BE Offset Ticks",Order=19,GroupName="1c. NY-A Short")] [Browsable(false)] public int NyAShortBreakevenOffsetTicks {get;set;}
        [NinjaScriptProperty][Range(0,200)][Display(Name="Max Bars In Trade",Order=20,GroupName="1c. NY-A Short")] [Browsable(false)] public int NyAShortMaxBarsInTrade {get;set;}
        [NinjaScriptProperty][Display(Name="Enable PriceOffset Trail",Order=21,GroupName="1c. NY-A Short")] [Browsable(false)] public bool NyAShortEnablePriceOffsetTrail {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="PriceOffset Reduction",Order=22,GroupName="1c. NY-A Short")] [Browsable(false)] public int NyAShortPriceOffsetReductionTicks {get;set;}
        [NinjaScriptProperty][Display(Name="Enable WMA Filter",Order=29,GroupName="1c. NY-A Short")] [Browsable(false)] public bool NyAShortEnableWmaFilter {get;set;}
        [NinjaScriptProperty][Range(1,500)][Display(Name="WMA Period",Order=30,GroupName="1c. NY-A Short")] [Browsable(false)] public int NyAShortWmaPeriod {get;set;}
        [NinjaScriptProperty][Range(0,100)][Display(Name="Min Body %",Order=32,GroupName="1c. NY-A Short")] [Browsable(false)] public double NyAShortMinBodyPct {get;set;}
        [NinjaScriptProperty][Range(0,200)][Display(Name="Trend Confirm Bars",Order=33,GroupName="1c. NY-A Short")] [Browsable(false)] public int NyAShortTrendConfirmBars {get;set;}
        [NinjaScriptProperty][Display(Name="Enable ADX Filter",Order=35,GroupName="1c. NY-A Short")] [Browsable(false)] public bool NyAShortEnableAdxFilter {get;set;}
        [NinjaScriptProperty][Range(1,100)][Display(Name="ADX Period",Order=36,GroupName="1c. NY-A Short")] [Browsable(false)] public int NyAShortAdxPeriod {get;set;}
        [NinjaScriptProperty][Range(0.0,100.0)][Display(Name="ADX Min Level",Order=37,GroupName="1c. NY-A Short")] [Browsable(false)] public double NyAShortAdxMinLevel {get;set;}
        [NinjaScriptProperty][Range(0.0,100.0)][Display(Name="ADX Max Level",Order=38,GroupName="1c. NY-A Short")] [Browsable(false)] public double NyAShortAdxMaxLevel {get;set;}
        [NinjaScriptProperty][Range(0.0,100.0)][Display(Name="ADX Peak Drawdown",Order=39,GroupName="1c. NY-A Short")] [Browsable(false)] public double NyAShortAdxPeakDrawdown {get;set;}

        // ── NY-B ────────────────────────────────────────────────────────────────
        [NinjaScriptProperty][Display(Name="Enable",Order=1,GroupName="2a. NY-B Session")] [Browsable(false)] public bool NyBEnable {get;set;}
        [NinjaScriptProperty][PropertyEditor("NinjaTrader.Gui.Tools.TimeEditorKey")][Display(Name="Trade Window Start",Order=2,GroupName="2a. NY-B Session")] [Browsable(false)] public DateTime NyBTradeWindowStart {get;set;}
        [NinjaScriptProperty][Display(Name="Enable No-New-Trades After",Order=3,GroupName="2a. NY-B Session")] [Browsable(false)] public bool NyBEnableNoNewTradesAfter {get;set;}
        [NinjaScriptProperty][PropertyEditor("NinjaTrader.Gui.Tools.TimeEditorKey")][Display(Name="No New Trades After",Order=4,GroupName="2a. NY-B Session")] [Browsable(false)] public DateTime NyBNoNewTradesAfter {get;set;}
        [NinjaScriptProperty][Display(Name="Enable Forced Close",Order=5,GroupName="2a. NY-B Session")] [Browsable(false)] public bool NyBEnableForcedClose {get;set;}
        [NinjaScriptProperty][PropertyEditor("NinjaTrader.Gui.Tools.TimeEditorKey")][Display(Name="Forced Close Time",Order=6,GroupName="2a. NY-B Session")] [Browsable(false)] public DateTime NyBForcedCloseTime {get;set;}
        [NinjaScriptProperty][Range(1,9999)][Display(Name="Max Profit Ticks",Order=10,GroupName="2a. NY-B Session")] [Browsable(false)] public int NyBMaxSessionProfitTicks {get;set;}
        [NinjaScriptProperty][Range(1,9999)][Display(Name="Max Loss Ticks",Order=11,GroupName="2a. NY-B Session")] [Browsable(false)] public int NyBMaxSessionLossTicks {get;set;}
                [NinjaScriptProperty][Range(1,20)][Display(Name="Max Trades",Order=8,GroupName="2a. NY-B Session")] [Browsable(false)] public int NyBMaxTradesPerSession {get;set;}
        [NinjaScriptProperty][Range(0,20)][Display(Name="Max Losses",Order=9,GroupName="2a. NY-B Session")] [Browsable(false)] public int NyBMaxLossesPerSession {get;set;}
        [NinjaScriptProperty][Display(Name="Require Direction Flip",Order=15,GroupName="2a. NY-B Session")] [Browsable(false)] public bool NyBRequireDirectionFlip {get;set;}
        [NinjaScriptProperty][Display(Name="Allow Same Dir After Loss",Order=16,GroupName="2a. NY-B Session")] [Browsable(false)] public bool NyBAllowSameDirectionAfterLoss {get;set;}
        [NinjaScriptProperty][Range(0,200)][Display(Name="Engulfing Exit After Bars",Order=17,GroupName="2a. NY-B Session")] [Browsable(false)] public int NyBEngulfingExitAfterBars {get;set;}
        [NinjaScriptProperty][Range(0,50)][Display(Name="Engulfing Exit Padding Ticks",Order=18,GroupName="2a. NY-B Session")] [Browsable(false)] public int NyBEngulfingExitPaddingTicks {get;set;}
        [NinjaScriptProperty][Range(0.01,100)][Display(Name="Candle Multiplier",Order=4,GroupName="2b. NY-B Long")] [Browsable(false)] public double NyBLongCandleMultiplier {get;set;}
        [NinjaScriptProperty][Range(0,9999)][Display(Name="Max TP Ticks",Order=5,GroupName="2b. NY-B Long")] [Browsable(false)] public int NyBLongMaxTakeProfitTicks {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="SL Extra Ticks",Order=6,GroupName="2b. NY-B Long")] [Browsable(false)] public int NyBLongSlExtraTicks {get;set;}
        [NinjaScriptProperty][Range(0,9999)][Display(Name="Max SL Ticks",Order=7,GroupName="2b. NY-B Long")] [Browsable(false)] public int NyBLongMaxSlTicks {get;set;}
        [NinjaScriptProperty][Range(0,10)][Display(Name="Max SL:TP Ratio",Order=8,GroupName="2b. NY-B Long")] [Browsable(false)] public double NyBLongMaxSlToTpRatio {get;set;}
        [NinjaScriptProperty][Display(Name="Use Prior Candle SL",Order=9,GroupName="2b. NY-B Long")] [Browsable(false)] public bool NyBLongUsePriorCandleSl {get;set;}
        [NinjaScriptProperty][Display(Name="SL At MA",Order=10,GroupName="2b. NY-B Long")] [Browsable(false)] public bool NyBLongSlAtMa {get;set;}
        [NinjaScriptProperty][Display(Name="Move SL To Entry Bar",Order=11,GroupName="2b. NY-B Long")] [Browsable(false)] public bool NyBLongMoveSlToEntryBar {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="Trail Offset Ticks",Order=12,GroupName="2b. NY-B Long")] [Browsable(false)] public int NyBLongTrailOffsetTicks {get;set;}
        [NinjaScriptProperty][Range(0,100)][Display(Name="Trail Delay Bars",Order=13,GroupName="2b. NY-B Long")] [Browsable(false)] public int NyBLongTrailDelayBars {get;set;}
        [NinjaScriptProperty][Range(0,500)][Display(Name="Trail Candle Offset",Order=14,GroupName="2b. NY-B Long")] [Browsable(false)] public int NyBLongTrailCandleOffset {get;set;}
        [NinjaScriptProperty][Display(Name="Enable Breakeven",Order=15,GroupName="2b. NY-B Long")] [Browsable(false)] public bool NyBLongEnableBreakeven {get;set;}
        [NinjaScriptProperty][Range(1,9999)][Display(Name="BE Trigger Ticks",Order=17,GroupName="2b. NY-B Long")] [Browsable(false)] public int NyBLongBreakevenTriggerTicks {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="BE Offset Ticks",Order=19,GroupName="2b. NY-B Long")] [Browsable(false)] public int NyBLongBreakevenOffsetTicks {get;set;}
        [NinjaScriptProperty][Range(0,200)][Display(Name="Max Bars In Trade",Order=20,GroupName="2b. NY-B Long")] [Browsable(false)] public int NyBLongMaxBarsInTrade {get;set;}
        [NinjaScriptProperty][Display(Name="Enable PriceOffset Trail",Order=21,GroupName="2b. NY-B Long")] [Browsable(false)] public bool NyBLongEnablePriceOffsetTrail {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="PriceOffset Reduction",Order=22,GroupName="2b. NY-B Long")] [Browsable(false)] public int NyBLongPriceOffsetReductionTicks {get;set;}
        [NinjaScriptProperty][Display(Name="Enable WMA Filter",Order=29,GroupName="2b. NY-B Long")] [Browsable(false)] public bool NyBLongEnableWmaFilter {get;set;}
        [NinjaScriptProperty][Range(1,500)][Display(Name="WMA Period",Order=30,GroupName="2b. NY-B Long")] [Browsable(false)] public int NyBLongWmaPeriod {get;set;}
        [NinjaScriptProperty][Range(0,100)][Display(Name="Min Body %",Order=32,GroupName="2b. NY-B Long")] [Browsable(false)] public double NyBLongMinBodyPct {get;set;}
        [NinjaScriptProperty][Range(0,200)][Display(Name="Trend Confirm Bars",Order=33,GroupName="2b. NY-B Long")] [Browsable(false)] public int NyBLongTrendConfirmBars {get;set;}
        [NinjaScriptProperty][Display(Name="Enable ADX Filter",Order=35,GroupName="2b. NY-B Long")] [Browsable(false)] public bool NyBLongEnableAdxFilter {get;set;}
        [NinjaScriptProperty][Range(1,100)][Display(Name="ADX Period",Order=36,GroupName="2b. NY-B Long")] [Browsable(false)] public int NyBLongAdxPeriod {get;set;}
        [NinjaScriptProperty][Range(0.0,100.0)][Display(Name="ADX Min Level",Order=37,GroupName="2b. NY-B Long")] [Browsable(false)] public double NyBLongAdxMinLevel {get;set;}
        [NinjaScriptProperty][Range(0.0,100.0)][Display(Name="ADX Max Level",Order=38,GroupName="2b. NY-B Long")] [Browsable(false)] public double NyBLongAdxMaxLevel {get;set;}
        [NinjaScriptProperty][Range(0.0,100.0)][Display(Name="ADX Peak Drawdown",Order=39,GroupName="2b. NY-B Long")] [Browsable(false)] public double NyBLongAdxPeakDrawdown {get;set;}
        [NinjaScriptProperty][Range(0.01,100)][Display(Name="Candle Multiplier",Order=4,GroupName="2c. NY-B Short")] [Browsable(false)] public double NyBShortCandleMultiplier {get;set;}
        [NinjaScriptProperty][Range(0,9999)][Display(Name="Max TP Ticks",Order=5,GroupName="2c. NY-B Short")] [Browsable(false)] public int NyBShortMaxTakeProfitTicks {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="SL Extra Ticks",Order=6,GroupName="2c. NY-B Short")] [Browsable(false)] public int NyBShortSlExtraTicks {get;set;}
        [NinjaScriptProperty][Range(0,9999)][Display(Name="Max SL Ticks",Order=7,GroupName="2c. NY-B Short")] [Browsable(false)] public int NyBShortMaxSlTicks {get;set;}
        [NinjaScriptProperty][Range(0,10)][Display(Name="Max SL:TP Ratio",Order=8,GroupName="2c. NY-B Short")] [Browsable(false)] public double NyBShortMaxSlToTpRatio {get;set;}
        [NinjaScriptProperty][Display(Name="Use Prior Candle SL",Order=9,GroupName="2c. NY-B Short")] [Browsable(false)] public bool NyBShortUsePriorCandleSl {get;set;}
        [NinjaScriptProperty][Display(Name="SL At MA",Order=10,GroupName="2c. NY-B Short")] [Browsable(false)] public bool NyBShortSlAtMa {get;set;}
        [NinjaScriptProperty][Display(Name="Move SL To Entry Bar",Order=11,GroupName="2c. NY-B Short")] [Browsable(false)] public bool NyBShortMoveSlToEntryBar {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="Trail Offset Ticks",Order=12,GroupName="2c. NY-B Short")] [Browsable(false)] public int NyBShortTrailOffsetTicks {get;set;}
        [NinjaScriptProperty][Range(0,100)][Display(Name="Trail Delay Bars",Order=13,GroupName="2c. NY-B Short")] [Browsable(false)] public int NyBShortTrailDelayBars {get;set;}
        [NinjaScriptProperty][Range(0,500)][Display(Name="Trail Candle Offset",Order=14,GroupName="2c. NY-B Short")] [Browsable(false)] public int NyBShortTrailCandleOffset {get;set;}
        [NinjaScriptProperty][Display(Name="Enable Breakeven",Order=15,GroupName="2c. NY-B Short")] [Browsable(false)] public bool NyBShortEnableBreakeven {get;set;}
        [NinjaScriptProperty][Range(1,9999)][Display(Name="BE Trigger Ticks",Order=17,GroupName="2c. NY-B Short")] [Browsable(false)] public int NyBShortBreakevenTriggerTicks {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="BE Offset Ticks",Order=19,GroupName="2c. NY-B Short")] [Browsable(false)] public int NyBShortBreakevenOffsetTicks {get;set;}
        [NinjaScriptProperty][Range(0,200)][Display(Name="Max Bars In Trade",Order=20,GroupName="2c. NY-B Short")] [Browsable(false)] public int NyBShortMaxBarsInTrade {get;set;}
        [NinjaScriptProperty][Display(Name="Enable PriceOffset Trail",Order=21,GroupName="2c. NY-B Short")] [Browsable(false)] public bool NyBShortEnablePriceOffsetTrail {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="PriceOffset Reduction",Order=22,GroupName="2c. NY-B Short")] [Browsable(false)] public int NyBShortPriceOffsetReductionTicks {get;set;}
        [NinjaScriptProperty][Display(Name="Enable WMA Filter",Order=29,GroupName="2c. NY-B Short")] [Browsable(false)] public bool NyBShortEnableWmaFilter {get;set;}
        [NinjaScriptProperty][Range(1,500)][Display(Name="WMA Period",Order=30,GroupName="2c. NY-B Short")] [Browsable(false)] public int NyBShortWmaPeriod {get;set;}
        [NinjaScriptProperty][Range(0,100)][Display(Name="Min Body %",Order=32,GroupName="2c. NY-B Short")] [Browsable(false)] public double NyBShortMinBodyPct {get;set;}
        [NinjaScriptProperty][Range(0,200)][Display(Name="Trend Confirm Bars",Order=33,GroupName="2c. NY-B Short")] [Browsable(false)] public int NyBShortTrendConfirmBars {get;set;}
        [NinjaScriptProperty][Display(Name="Enable ADX Filter",Order=35,GroupName="2c. NY-B Short")] [Browsable(false)] public bool NyBShortEnableAdxFilter {get;set;}
        [NinjaScriptProperty][Range(1,100)][Display(Name="ADX Period",Order=36,GroupName="2c. NY-B Short")] [Browsable(false)] public int NyBShortAdxPeriod {get;set;}
        [NinjaScriptProperty][Range(0.0,100.0)][Display(Name="ADX Min Level",Order=37,GroupName="2c. NY-B Short")] [Browsable(false)] public double NyBShortAdxMinLevel {get;set;}
        [NinjaScriptProperty][Range(0.0,100.0)][Display(Name="ADX Max Level",Order=38,GroupName="2c. NY-B Short")] [Browsable(false)] public double NyBShortAdxMaxLevel {get;set;}
        [NinjaScriptProperty][Range(0.0,100.0)][Display(Name="ADX Peak Drawdown",Order=39,GroupName="2c. NY-B Short")] [Browsable(false)] public double NyBShortAdxPeakDrawdown {get;set;}
        // ── NY-C ────────────────────────────────────────────────────────────────
        [NinjaScriptProperty][Display(Name="Enable",Order=1,GroupName="3a. NY-C Session")] [Browsable(false)] public bool NyCEnable {get;set;}
        [NinjaScriptProperty][PropertyEditor("NinjaTrader.Gui.Tools.TimeEditorKey")][Display(Name="Trade Window Start",Order=2,GroupName="3a. NY-C Session")] [Browsable(false)] public DateTime NyCTradeWindowStart {get;set;}
        [NinjaScriptProperty][Display(Name="Enable No-New-Trades After",Order=3,GroupName="3a. NY-C Session")] [Browsable(false)] public bool NyCEnableNoNewTradesAfter {get;set;}
        [NinjaScriptProperty][PropertyEditor("NinjaTrader.Gui.Tools.TimeEditorKey")][Display(Name="No New Trades After",Order=4,GroupName="3a. NY-C Session")] [Browsable(false)] public DateTime NyCNoNewTradesAfter {get;set;}
        [NinjaScriptProperty][Display(Name="Enable Forced Close",Order=5,GroupName="3a. NY-C Session")] [Browsable(false)] public bool NyCEnableForcedClose {get;set;}
        [NinjaScriptProperty][PropertyEditor("NinjaTrader.Gui.Tools.TimeEditorKey")][Display(Name="Forced Close Time",Order=6,GroupName="3a. NY-C Session")] [Browsable(false)] public DateTime NyCForcedCloseTime {get;set;}
        [NinjaScriptProperty][Range(1,9999)][Display(Name="Max Profit Ticks",Order=10,GroupName="3a. NY-C Session")] [Browsable(false)] public int NyCMaxSessionProfitTicks {get;set;}
        [NinjaScriptProperty][Range(1,9999)][Display(Name="Max Loss Ticks",Order=11,GroupName="3a. NY-C Session")] [Browsable(false)] public int NyCMaxSessionLossTicks {get;set;}
                [NinjaScriptProperty][Range(1,20)][Display(Name="Max Trades",Order=8,GroupName="3a. NY-C Session")] [Browsable(false)] public int NyCMaxTradesPerSession {get;set;}
        [NinjaScriptProperty][Range(0,20)][Display(Name="Max Losses",Order=9,GroupName="3a. NY-C Session")] [Browsable(false)] public int NyCMaxLossesPerSession {get;set;}
        [NinjaScriptProperty][Display(Name="Require Direction Flip",Order=15,GroupName="3a. NY-C Session")] [Browsable(false)] public bool NyCRequireDirectionFlip {get;set;}
        [NinjaScriptProperty][Display(Name="Allow Same Dir After Loss",Order=16,GroupName="3a. NY-C Session")] [Browsable(false)] public bool NyCAllowSameDirectionAfterLoss {get;set;}
        [NinjaScriptProperty][Range(0,200)][Display(Name="Engulfing Exit After Bars",Order=17,GroupName="3a. NY-C Session")] [Browsable(false)] public int NyCEngulfingExitAfterBars {get;set;}
        [NinjaScriptProperty][Range(0,50)][Display(Name="Engulfing Exit Padding Ticks",Order=18,GroupName="3a. NY-C Session")] [Browsable(false)] public int NyCEngulfingExitPaddingTicks {get;set;}
        [NinjaScriptProperty][Range(0.01,100)][Display(Name="Candle Multiplier",Order=4,GroupName="3b. NY-C Long")] [Browsable(false)] public double NyCLongCandleMultiplier {get;set;}
        [NinjaScriptProperty][Range(0,9999)][Display(Name="Max TP Ticks",Order=5,GroupName="3b. NY-C Long")] [Browsable(false)] public int NyCLongMaxTakeProfitTicks {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="SL Extra Ticks",Order=6,GroupName="3b. NY-C Long")] [Browsable(false)] public int NyCLongSlExtraTicks {get;set;}
        [NinjaScriptProperty][Range(0,9999)][Display(Name="Max SL Ticks",Order=7,GroupName="3b. NY-C Long")] [Browsable(false)] public int NyCLongMaxSlTicks {get;set;}
        [NinjaScriptProperty][Range(0,10)][Display(Name="Max SL:TP Ratio",Order=8,GroupName="3b. NY-C Long")] [Browsable(false)] public double NyCLongMaxSlToTpRatio {get;set;}
        [NinjaScriptProperty][Display(Name="Use Prior Candle SL",Order=9,GroupName="3b. NY-C Long")] [Browsable(false)] public bool NyCLongUsePriorCandleSl {get;set;}
        [NinjaScriptProperty][Display(Name="SL At MA",Order=10,GroupName="3b. NY-C Long")] [Browsable(false)] public bool NyCLongSlAtMa {get;set;}
        [NinjaScriptProperty][Display(Name="Move SL To Entry Bar",Order=11,GroupName="3b. NY-C Long")] [Browsable(false)] public bool NyCLongMoveSlToEntryBar {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="Trail Offset Ticks",Order=12,GroupName="3b. NY-C Long")] [Browsable(false)] public int NyCLongTrailOffsetTicks {get;set;}
        [NinjaScriptProperty][Range(0,100)][Display(Name="Trail Delay Bars",Order=13,GroupName="3b. NY-C Long")] [Browsable(false)] public int NyCLongTrailDelayBars {get;set;}
        [NinjaScriptProperty][Range(0,500)][Display(Name="Trail Candle Offset",Order=14,GroupName="3b. NY-C Long")] [Browsable(false)] public int NyCLongTrailCandleOffset {get;set;}
        [NinjaScriptProperty][Display(Name="Enable Breakeven",Order=15,GroupName="3b. NY-C Long")] [Browsable(false)] public bool NyCLongEnableBreakeven {get;set;}
        [NinjaScriptProperty][Range(1,9999)][Display(Name="BE Trigger Ticks",Order=17,GroupName="3b. NY-C Long")] [Browsable(false)] public int NyCLongBreakevenTriggerTicks {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="BE Offset Ticks",Order=19,GroupName="3b. NY-C Long")] [Browsable(false)] public int NyCLongBreakevenOffsetTicks {get;set;}
        [NinjaScriptProperty][Range(0,200)][Display(Name="Max Bars In Trade",Order=20,GroupName="3b. NY-C Long")] [Browsable(false)] public int NyCLongMaxBarsInTrade {get;set;}
        [NinjaScriptProperty][Display(Name="Enable PriceOffset Trail",Order=21,GroupName="3b. NY-C Long")] [Browsable(false)] public bool NyCLongEnablePriceOffsetTrail {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="PriceOffset Reduction",Order=22,GroupName="3b. NY-C Long")] [Browsable(false)] public int NyCLongPriceOffsetReductionTicks {get;set;}
        [NinjaScriptProperty][Display(Name="Enable WMA Filter",Order=29,GroupName="3b. NY-C Long")] [Browsable(false)] public bool NyCLongEnableWmaFilter {get;set;}
        [NinjaScriptProperty][Range(1,500)][Display(Name="WMA Period",Order=30,GroupName="3b. NY-C Long")] [Browsable(false)] public int NyCLongWmaPeriod {get;set;}
        [NinjaScriptProperty][Range(0,100)][Display(Name="Min Body %",Order=32,GroupName="3b. NY-C Long")] [Browsable(false)] public double NyCLongMinBodyPct {get;set;}
        [NinjaScriptProperty][Range(0,200)][Display(Name="Trend Confirm Bars",Order=33,GroupName="3b. NY-C Long")] [Browsable(false)] public int NyCLongTrendConfirmBars {get;set;}
        [NinjaScriptProperty][Display(Name="Enable ADX Filter",Order=35,GroupName="3b. NY-C Long")] [Browsable(false)] public bool NyCLongEnableAdxFilter {get;set;}
        [NinjaScriptProperty][Range(1,100)][Display(Name="ADX Period",Order=36,GroupName="3b. NY-C Long")] [Browsable(false)] public int NyCLongAdxPeriod {get;set;}
        [NinjaScriptProperty][Range(0.0,100.0)][Display(Name="ADX Min Level",Order=37,GroupName="3b. NY-C Long")] [Browsable(false)] public double NyCLongAdxMinLevel {get;set;}
        [NinjaScriptProperty][Range(0.0,100.0)][Display(Name="ADX Max Level",Order=38,GroupName="3b. NY-C Long")] [Browsable(false)] public double NyCLongAdxMaxLevel {get;set;}
        [NinjaScriptProperty][Range(0.0,100.0)][Display(Name="ADX Peak Drawdown",Order=39,GroupName="3b. NY-C Long")] [Browsable(false)] public double NyCLongAdxPeakDrawdown {get;set;}
        [NinjaScriptProperty][Range(0.01,100)][Display(Name="Candle Multiplier",Order=4,GroupName="3c. NY-C Short")] [Browsable(false)] public double NyCShortCandleMultiplier {get;set;}
        [NinjaScriptProperty][Range(0,9999)][Display(Name="Max TP Ticks",Order=5,GroupName="3c. NY-C Short")] [Browsable(false)] public int NyCShortMaxTakeProfitTicks {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="SL Extra Ticks",Order=6,GroupName="3c. NY-C Short")] [Browsable(false)] public int NyCShortSlExtraTicks {get;set;}
        [NinjaScriptProperty][Range(0,9999)][Display(Name="Max SL Ticks",Order=7,GroupName="3c. NY-C Short")] [Browsable(false)] public int NyCShortMaxSlTicks {get;set;}
        [NinjaScriptProperty][Range(0,10)][Display(Name="Max SL:TP Ratio",Order=8,GroupName="3c. NY-C Short")] [Browsable(false)] public double NyCShortMaxSlToTpRatio {get;set;}
        [NinjaScriptProperty][Display(Name="Use Prior Candle SL",Order=9,GroupName="3c. NY-C Short")] [Browsable(false)] public bool NyCShortUsePriorCandleSl {get;set;}
        [NinjaScriptProperty][Display(Name="SL At MA",Order=10,GroupName="3c. NY-C Short")] [Browsable(false)] public bool NyCShortSlAtMa {get;set;}
        [NinjaScriptProperty][Display(Name="Move SL To Entry Bar",Order=11,GroupName="3c. NY-C Short")] [Browsable(false)] public bool NyCShortMoveSlToEntryBar {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="Trail Offset Ticks",Order=12,GroupName="3c. NY-C Short")] [Browsable(false)] public int NyCShortTrailOffsetTicks {get;set;}
        [NinjaScriptProperty][Range(0,100)][Display(Name="Trail Delay Bars",Order=13,GroupName="3c. NY-C Short")] [Browsable(false)] public int NyCShortTrailDelayBars {get;set;}
        [NinjaScriptProperty][Range(0,500)][Display(Name="Trail Candle Offset",Order=14,GroupName="3c. NY-C Short")] [Browsable(false)] public int NyCShortTrailCandleOffset {get;set;}
        [NinjaScriptProperty][Display(Name="Enable Breakeven",Order=15,GroupName="3c. NY-C Short")] [Browsable(false)] public bool NyCShortEnableBreakeven {get;set;}
        [NinjaScriptProperty][Range(1,9999)][Display(Name="BE Trigger Ticks",Order=17,GroupName="3c. NY-C Short")] [Browsable(false)] public int NyCShortBreakevenTriggerTicks {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="BE Offset Ticks",Order=19,GroupName="3c. NY-C Short")] [Browsable(false)] public int NyCShortBreakevenOffsetTicks {get;set;}
        [NinjaScriptProperty][Range(0,200)][Display(Name="Max Bars In Trade",Order=20,GroupName="3c. NY-C Short")] [Browsable(false)] public int NyCShortMaxBarsInTrade {get;set;}
        [NinjaScriptProperty][Display(Name="Enable PriceOffset Trail",Order=21,GroupName="3c. NY-C Short")] [Browsable(false)] public bool NyCShortEnablePriceOffsetTrail {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="PriceOffset Reduction",Order=22,GroupName="3c. NY-C Short")] [Browsable(false)] public int NyCShortPriceOffsetReductionTicks {get;set;}
        [NinjaScriptProperty][Display(Name="Enable WMA Filter",Order=29,GroupName="3c. NY-C Short")] [Browsable(false)] public bool NyCShortEnableWmaFilter {get;set;}
        [NinjaScriptProperty][Range(1,500)][Display(Name="WMA Period",Order=30,GroupName="3c. NY-C Short")] [Browsable(false)] public int NyCShortWmaPeriod {get;set;}
        [NinjaScriptProperty][Range(0,100)][Display(Name="Min Body %",Order=32,GroupName="3c. NY-C Short")] [Browsable(false)] public double NyCShortMinBodyPct {get;set;}
        [NinjaScriptProperty][Range(0,200)][Display(Name="Trend Confirm Bars",Order=33,GroupName="3c. NY-C Short")] [Browsable(false)] public int NyCShortTrendConfirmBars {get;set;}
        [NinjaScriptProperty][Display(Name="Enable ADX Filter",Order=35,GroupName="3c. NY-C Short")] [Browsable(false)] public bool NyCShortEnableAdxFilter {get;set;}
        [NinjaScriptProperty][Range(1,100)][Display(Name="ADX Period",Order=36,GroupName="3c. NY-C Short")] [Browsable(false)] public int NyCShortAdxPeriod {get;set;}
        [NinjaScriptProperty][Range(0.0,100.0)][Display(Name="ADX Min Level",Order=37,GroupName="3c. NY-C Short")] [Browsable(false)] public double NyCShortAdxMinLevel {get;set;}
        [NinjaScriptProperty][Range(0.0,100.0)][Display(Name="ADX Max Level",Order=38,GroupName="3c. NY-C Short")] [Browsable(false)] public double NyCShortAdxMaxLevel {get;set;}
        [NinjaScriptProperty][Range(0.0,100.0)][Display(Name="ADX Peak Drawdown",Order=39,GroupName="3c. NY-C Short")] [Browsable(false)] public double NyCShortAdxPeakDrawdown {get;set;}
        // ── EU-A ────────────────────────────────────────────────────────────────
        [NinjaScriptProperty][Display(Name="Enable",Order=1,GroupName="4a. EU-A Session")] [Browsable(false)] public bool EuAEnable {get;set;}
        [NinjaScriptProperty][PropertyEditor("NinjaTrader.Gui.Tools.TimeEditorKey")][Display(Name="Trade Window Start",Order=2,GroupName="4a. EU-A Session")] [Browsable(false)] public DateTime EuATradeWindowStart {get;set;}
        [NinjaScriptProperty][Display(Name="Enable No-New-Trades After",Order=3,GroupName="4a. EU-A Session")] [Browsable(false)] public bool EuAEnableNoNewTradesAfter {get;set;}
        [NinjaScriptProperty][PropertyEditor("NinjaTrader.Gui.Tools.TimeEditorKey")][Display(Name="No New Trades After",Order=4,GroupName="4a. EU-A Session")] [Browsable(false)] public DateTime EuANoNewTradesAfter {get;set;}
        [NinjaScriptProperty][Display(Name="Enable Forced Close",Order=5,GroupName="4a. EU-A Session")] [Browsable(false)] public bool EuAEnableForcedClose {get;set;}
        [NinjaScriptProperty][PropertyEditor("NinjaTrader.Gui.Tools.TimeEditorKey")][Display(Name="Forced Close Time",Order=6,GroupName="4a. EU-A Session")] [Browsable(false)] public DateTime EuAForcedCloseTime {get;set;}
        [NinjaScriptProperty][Range(1,9999)][Display(Name="Max Profit Ticks",Order=10,GroupName="4a. EU-A Session")] [Browsable(false)] public int EuAMaxSessionProfitTicks {get;set;}
        [NinjaScriptProperty][Range(1,9999)][Display(Name="Max Loss Ticks",Order=11,GroupName="4a. EU-A Session")] [Browsable(false)] public int EuAMaxSessionLossTicks {get;set;}
                [NinjaScriptProperty][Range(1,20)][Display(Name="Max Trades",Order=8,GroupName="4a. EU-A Session")] [Browsable(false)] public int EuAMaxTradesPerSession {get;set;}
        [NinjaScriptProperty][Range(0,20)][Display(Name="Max Losses",Order=9,GroupName="4a. EU-A Session")] [Browsable(false)] public int EuAMaxLossesPerSession {get;set;}
        [NinjaScriptProperty][Display(Name="Require Direction Flip",Order=15,GroupName="4a. EU-A Session")] [Browsable(false)] public bool EuARequireDirectionFlip {get;set;}
        [NinjaScriptProperty][Display(Name="Allow Same Dir After Loss",Order=16,GroupName="4a. EU-A Session")] [Browsable(false)] public bool EuAAllowSameDirectionAfterLoss {get;set;}
        [NinjaScriptProperty][Range(0,200)][Display(Name="Engulfing Exit After Bars",Order=17,GroupName="4a. EU-A Session")] [Browsable(false)] public int EuAEngulfingExitAfterBars {get;set;}
        [NinjaScriptProperty][Range(0,50)][Display(Name="Engulfing Exit Padding Ticks",Order=18,GroupName="4a. EU-A Session")] [Browsable(false)] public int EuAEngulfingExitPaddingTicks {get;set;}
        [NinjaScriptProperty][Range(0.01,100)][Display(Name="Candle Multiplier",Order=4,GroupName="4b. EU-A Long")] [Browsable(false)] public double EuALongCandleMultiplier {get;set;}
        [NinjaScriptProperty][Range(0,9999)][Display(Name="Max TP Ticks",Order=5,GroupName="4b. EU-A Long")] [Browsable(false)] public int EuALongMaxTakeProfitTicks {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="SL Extra Ticks",Order=6,GroupName="4b. EU-A Long")] [Browsable(false)] public int EuALongSlExtraTicks {get;set;}
        [NinjaScriptProperty][Range(0,9999)][Display(Name="Max SL Ticks",Order=7,GroupName="4b. EU-A Long")] [Browsable(false)] public int EuALongMaxSlTicks {get;set;}
        [NinjaScriptProperty][Range(0,10)][Display(Name="Max SL:TP Ratio",Order=8,GroupName="4b. EU-A Long")] [Browsable(false)] public double EuALongMaxSlToTpRatio {get;set;}
        [NinjaScriptProperty][Display(Name="Use Prior Candle SL",Order=9,GroupName="4b. EU-A Long")] [Browsable(false)] public bool EuALongUsePriorCandleSl {get;set;}
        [NinjaScriptProperty][Display(Name="SL At MA",Order=10,GroupName="4b. EU-A Long")] [Browsable(false)] public bool EuALongSlAtMa {get;set;}
        [NinjaScriptProperty][Display(Name="Move SL To Entry Bar",Order=11,GroupName="4b. EU-A Long")] [Browsable(false)] public bool EuALongMoveSlToEntryBar {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="Trail Offset Ticks",Order=12,GroupName="4b. EU-A Long")] [Browsable(false)] public int EuALongTrailOffsetTicks {get;set;}
        [NinjaScriptProperty][Range(0,100)][Display(Name="Trail Delay Bars",Order=13,GroupName="4b. EU-A Long")] [Browsable(false)] public int EuALongTrailDelayBars {get;set;}
        [NinjaScriptProperty][Range(0,500)][Display(Name="Trail Candle Offset",Order=14,GroupName="4b. EU-A Long")] [Browsable(false)] public int EuALongTrailCandleOffset {get;set;}
        [NinjaScriptProperty][Display(Name="Enable Breakeven",Order=15,GroupName="4b. EU-A Long")] [Browsable(false)] public bool EuALongEnableBreakeven {get;set;}
        [NinjaScriptProperty][Range(1,9999)][Display(Name="BE Trigger Ticks",Order=17,GroupName="4b. EU-A Long")] [Browsable(false)] public int EuALongBreakevenTriggerTicks {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="BE Offset Ticks",Order=19,GroupName="4b. EU-A Long")] [Browsable(false)] public int EuALongBreakevenOffsetTicks {get;set;}
        [NinjaScriptProperty][Range(0,200)][Display(Name="Max Bars In Trade",Order=20,GroupName="4b. EU-A Long")] [Browsable(false)] public int EuALongMaxBarsInTrade {get;set;}
        [NinjaScriptProperty][Display(Name="Enable PriceOffset Trail",Order=21,GroupName="4b. EU-A Long")] [Browsable(false)] public bool EuALongEnablePriceOffsetTrail {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="PriceOffset Reduction",Order=22,GroupName="4b. EU-A Long")] [Browsable(false)] public int EuALongPriceOffsetReductionTicks {get;set;}
        [NinjaScriptProperty][Display(Name="Enable WMA Filter",Order=29,GroupName="4b. EU-A Long")] [Browsable(false)] public bool EuALongEnableWmaFilter {get;set;}
        [NinjaScriptProperty][Range(1,500)][Display(Name="WMA Period",Order=30,GroupName="4b. EU-A Long")] [Browsable(false)] public int EuALongWmaPeriod {get;set;}
        [NinjaScriptProperty][Range(0,100)][Display(Name="Min Body %",Order=32,GroupName="4b. EU-A Long")] [Browsable(false)] public double EuALongMinBodyPct {get;set;}
        [NinjaScriptProperty][Range(0,200)][Display(Name="Trend Confirm Bars",Order=33,GroupName="4b. EU-A Long")] [Browsable(false)] public int EuALongTrendConfirmBars {get;set;}
        [NinjaScriptProperty][Display(Name="Enable ADX Filter",Order=35,GroupName="4b. EU-A Long")] [Browsable(false)] public bool EuALongEnableAdxFilter {get;set;}
        [NinjaScriptProperty][Range(1,100)][Display(Name="ADX Period",Order=36,GroupName="4b. EU-A Long")] [Browsable(false)] public int EuALongAdxPeriod {get;set;}
        [NinjaScriptProperty][Range(0.0,100.0)][Display(Name="ADX Min Level",Order=37,GroupName="4b. EU-A Long")] [Browsable(false)] public double EuALongAdxMinLevel {get;set;}
        [NinjaScriptProperty][Range(0.0,100.0)][Display(Name="ADX Max Level",Order=38,GroupName="4b. EU-A Long")] [Browsable(false)] public double EuALongAdxMaxLevel {get;set;}
        [NinjaScriptProperty][Range(0.0,100.0)][Display(Name="ADX Peak Drawdown",Order=39,GroupName="4b. EU-A Long")] [Browsable(false)] public double EuALongAdxPeakDrawdown {get;set;}
        [NinjaScriptProperty][Range(0.01,100)][Display(Name="Candle Multiplier",Order=4,GroupName="4c. EU-A Short")] [Browsable(false)] public double EuAShortCandleMultiplier {get;set;}
        [NinjaScriptProperty][Range(0,9999)][Display(Name="Max TP Ticks",Order=5,GroupName="4c. EU-A Short")] [Browsable(false)] public int EuAShortMaxTakeProfitTicks {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="SL Extra Ticks",Order=6,GroupName="4c. EU-A Short")] [Browsable(false)] public int EuAShortSlExtraTicks {get;set;}
        [NinjaScriptProperty][Range(0,9999)][Display(Name="Max SL Ticks",Order=7,GroupName="4c. EU-A Short")] [Browsable(false)] public int EuAShortMaxSlTicks {get;set;}
        [NinjaScriptProperty][Range(0,10)][Display(Name="Max SL:TP Ratio",Order=8,GroupName="4c. EU-A Short")] [Browsable(false)] public double EuAShortMaxSlToTpRatio {get;set;}
        [NinjaScriptProperty][Display(Name="Use Prior Candle SL",Order=9,GroupName="4c. EU-A Short")] [Browsable(false)] public bool EuAShortUsePriorCandleSl {get;set;}
        [NinjaScriptProperty][Display(Name="SL At MA",Order=10,GroupName="4c. EU-A Short")] [Browsable(false)] public bool EuAShortSlAtMa {get;set;}
        [NinjaScriptProperty][Display(Name="Move SL To Entry Bar",Order=11,GroupName="4c. EU-A Short")] [Browsable(false)] public bool EuAShortMoveSlToEntryBar {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="Trail Offset Ticks",Order=12,GroupName="4c. EU-A Short")] [Browsable(false)] public int EuAShortTrailOffsetTicks {get;set;}
        [NinjaScriptProperty][Range(0,100)][Display(Name="Trail Delay Bars",Order=13,GroupName="4c. EU-A Short")] [Browsable(false)] public int EuAShortTrailDelayBars {get;set;}
        [NinjaScriptProperty][Range(0,500)][Display(Name="Trail Candle Offset",Order=14,GroupName="4c. EU-A Short")] [Browsable(false)] public int EuAShortTrailCandleOffset {get;set;}
        [NinjaScriptProperty][Display(Name="Enable Breakeven",Order=15,GroupName="4c. EU-A Short")] [Browsable(false)] public bool EuAShortEnableBreakeven {get;set;}
        [NinjaScriptProperty][Range(1,9999)][Display(Name="BE Trigger Ticks",Order=17,GroupName="4c. EU-A Short")] [Browsable(false)] public int EuAShortBreakevenTriggerTicks {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="BE Offset Ticks",Order=19,GroupName="4c. EU-A Short")] [Browsable(false)] public int EuAShortBreakevenOffsetTicks {get;set;}
        [NinjaScriptProperty][Range(0,200)][Display(Name="Max Bars In Trade",Order=20,GroupName="4c. EU-A Short")] [Browsable(false)] public int EuAShortMaxBarsInTrade {get;set;}
        [NinjaScriptProperty][Display(Name="Enable PriceOffset Trail",Order=21,GroupName="4c. EU-A Short")] [Browsable(false)] public bool EuAShortEnablePriceOffsetTrail {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="PriceOffset Reduction",Order=22,GroupName="4c. EU-A Short")] [Browsable(false)] public int EuAShortPriceOffsetReductionTicks {get;set;}
        [NinjaScriptProperty][Display(Name="Enable WMA Filter",Order=29,GroupName="4c. EU-A Short")] [Browsable(false)] public bool EuAShortEnableWmaFilter {get;set;}
        [NinjaScriptProperty][Range(1,500)][Display(Name="WMA Period",Order=30,GroupName="4c. EU-A Short")] [Browsable(false)] public int EuAShortWmaPeriod {get;set;}
        [NinjaScriptProperty][Range(0,100)][Display(Name="Min Body %",Order=32,GroupName="4c. EU-A Short")] [Browsable(false)] public double EuAShortMinBodyPct {get;set;}
        [NinjaScriptProperty][Range(0,200)][Display(Name="Trend Confirm Bars",Order=33,GroupName="4c. EU-A Short")] [Browsable(false)] public int EuAShortTrendConfirmBars {get;set;}
        [NinjaScriptProperty][Display(Name="Enable ADX Filter",Order=35,GroupName="4c. EU-A Short")] [Browsable(false)] public bool EuAShortEnableAdxFilter {get;set;}
        [NinjaScriptProperty][Range(1,100)][Display(Name="ADX Period",Order=36,GroupName="4c. EU-A Short")] [Browsable(false)] public int EuAShortAdxPeriod {get;set;}
        [NinjaScriptProperty][Range(0.0,100.0)][Display(Name="ADX Min Level",Order=37,GroupName="4c. EU-A Short")] [Browsable(false)] public double EuAShortAdxMinLevel {get;set;}
        [NinjaScriptProperty][Range(0.0,100.0)][Display(Name="ADX Max Level",Order=38,GroupName="4c. EU-A Short")] [Browsable(false)] public double EuAShortAdxMaxLevel {get;set;}
        [NinjaScriptProperty][Range(0.0,100.0)][Display(Name="ADX Peak Drawdown",Order=39,GroupName="4c. EU-A Short")] [Browsable(false)] public double EuAShortAdxPeakDrawdown {get;set;}
        // ── EU-B ────────────────────────────────────────────────────────────────
        [NinjaScriptProperty][Display(Name="Enable",Order=1,GroupName="5a. EU-B Session")] [Browsable(false)] public bool EuBEnable {get;set;}
        [NinjaScriptProperty][PropertyEditor("NinjaTrader.Gui.Tools.TimeEditorKey")][Display(Name="Trade Window Start",Order=2,GroupName="5a. EU-B Session")] [Browsable(false)] public DateTime EuBTradeWindowStart {get;set;}
        [NinjaScriptProperty][Display(Name="Enable No-New-Trades After",Order=3,GroupName="5a. EU-B Session")] [Browsable(false)] public bool EuBEnableNoNewTradesAfter {get;set;}
        [NinjaScriptProperty][PropertyEditor("NinjaTrader.Gui.Tools.TimeEditorKey")][Display(Name="No New Trades After",Order=4,GroupName="5a. EU-B Session")] [Browsable(false)] public DateTime EuBNoNewTradesAfter {get;set;}
        [NinjaScriptProperty][Display(Name="Enable Forced Close",Order=5,GroupName="5a. EU-B Session")] [Browsable(false)] public bool EuBEnableForcedClose {get;set;}
        [NinjaScriptProperty][PropertyEditor("NinjaTrader.Gui.Tools.TimeEditorKey")][Display(Name="Forced Close Time",Order=6,GroupName="5a. EU-B Session")] [Browsable(false)] public DateTime EuBForcedCloseTime {get;set;}
        [NinjaScriptProperty][Range(1,9999)][Display(Name="Max Profit Ticks",Order=10,GroupName="5a. EU-B Session")] [Browsable(false)] public int EuBMaxSessionProfitTicks {get;set;}
        [NinjaScriptProperty][Range(1,9999)][Display(Name="Max Loss Ticks",Order=11,GroupName="5a. EU-B Session")] [Browsable(false)] public int EuBMaxSessionLossTicks {get;set;}
                [NinjaScriptProperty][Range(1,20)][Display(Name="Max Trades",Order=8,GroupName="5a. EU-B Session")] [Browsable(false)] public int EuBMaxTradesPerSession {get;set;}
        [NinjaScriptProperty][Range(0,20)][Display(Name="Max Losses",Order=9,GroupName="5a. EU-B Session")] [Browsable(false)] public int EuBMaxLossesPerSession {get;set;}
        [NinjaScriptProperty][Display(Name="Require Direction Flip",Order=15,GroupName="5a. EU-B Session")] [Browsable(false)] public bool EuBRequireDirectionFlip {get;set;}
        [NinjaScriptProperty][Display(Name="Allow Same Dir After Loss",Order=16,GroupName="5a. EU-B Session")] [Browsable(false)] public bool EuBAllowSameDirectionAfterLoss {get;set;}
        [NinjaScriptProperty][Range(0,200)][Display(Name="Engulfing Exit After Bars",Order=17,GroupName="5a. EU-B Session")] [Browsable(false)] public int EuBEngulfingExitAfterBars {get;set;}
        [NinjaScriptProperty][Range(0,50)][Display(Name="Engulfing Exit Padding Ticks",Order=18,GroupName="5a. EU-B Session")] [Browsable(false)] public int EuBEngulfingExitPaddingTicks {get;set;}
        [NinjaScriptProperty][Range(0.01,100)][Display(Name="Candle Multiplier",Order=4,GroupName="5b. EU-B Long")] [Browsable(false)] public double EuBLongCandleMultiplier {get;set;}
        [NinjaScriptProperty][Range(0,9999)][Display(Name="Max TP Ticks",Order=5,GroupName="5b. EU-B Long")] [Browsable(false)] public int EuBLongMaxTakeProfitTicks {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="SL Extra Ticks",Order=6,GroupName="5b. EU-B Long")] [Browsable(false)] public int EuBLongSlExtraTicks {get;set;}
        [NinjaScriptProperty][Range(0,9999)][Display(Name="Max SL Ticks",Order=7,GroupName="5b. EU-B Long")] [Browsable(false)] public int EuBLongMaxSlTicks {get;set;}
        [NinjaScriptProperty][Range(0,10)][Display(Name="Max SL:TP Ratio",Order=8,GroupName="5b. EU-B Long")] [Browsable(false)] public double EuBLongMaxSlToTpRatio {get;set;}
        [NinjaScriptProperty][Display(Name="Use Prior Candle SL",Order=9,GroupName="5b. EU-B Long")] [Browsable(false)] public bool EuBLongUsePriorCandleSl {get;set;}
        [NinjaScriptProperty][Display(Name="SL At MA",Order=10,GroupName="5b. EU-B Long")] [Browsable(false)] public bool EuBLongSlAtMa {get;set;}
        [NinjaScriptProperty][Display(Name="Move SL To Entry Bar",Order=11,GroupName="5b. EU-B Long")] [Browsable(false)] public bool EuBLongMoveSlToEntryBar {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="Trail Offset Ticks",Order=12,GroupName="5b. EU-B Long")] [Browsable(false)] public int EuBLongTrailOffsetTicks {get;set;}
        [NinjaScriptProperty][Range(0,100)][Display(Name="Trail Delay Bars",Order=13,GroupName="5b. EU-B Long")] [Browsable(false)] public int EuBLongTrailDelayBars {get;set;}
        [NinjaScriptProperty][Range(0,500)][Display(Name="Trail Candle Offset",Order=14,GroupName="5b. EU-B Long")] [Browsable(false)] public int EuBLongTrailCandleOffset {get;set;}
        [NinjaScriptProperty][Display(Name="Enable Breakeven",Order=15,GroupName="5b. EU-B Long")] [Browsable(false)] public bool EuBLongEnableBreakeven {get;set;}
        [NinjaScriptProperty][Range(1,9999)][Display(Name="BE Trigger Ticks",Order=17,GroupName="5b. EU-B Long")] [Browsable(false)] public int EuBLongBreakevenTriggerTicks {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="BE Offset Ticks",Order=19,GroupName="5b. EU-B Long")] [Browsable(false)] public int EuBLongBreakevenOffsetTicks {get;set;}
        [NinjaScriptProperty][Range(0,200)][Display(Name="Max Bars In Trade",Order=20,GroupName="5b. EU-B Long")] [Browsable(false)] public int EuBLongMaxBarsInTrade {get;set;}
        [NinjaScriptProperty][Display(Name="Enable PriceOffset Trail",Order=21,GroupName="5b. EU-B Long")] [Browsable(false)] public bool EuBLongEnablePriceOffsetTrail {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="PriceOffset Reduction",Order=22,GroupName="5b. EU-B Long")] [Browsable(false)] public int EuBLongPriceOffsetReductionTicks {get;set;}
        [NinjaScriptProperty][Display(Name="Enable WMA Filter",Order=29,GroupName="5b. EU-B Long")] [Browsable(false)] public bool EuBLongEnableWmaFilter {get;set;}
        [NinjaScriptProperty][Range(1,500)][Display(Name="WMA Period",Order=30,GroupName="5b. EU-B Long")] [Browsable(false)] public int EuBLongWmaPeriod {get;set;}
        [NinjaScriptProperty][Range(0,100)][Display(Name="Min Body %",Order=32,GroupName="5b. EU-B Long")] [Browsable(false)] public double EuBLongMinBodyPct {get;set;}
        [NinjaScriptProperty][Range(0,200)][Display(Name="Trend Confirm Bars",Order=33,GroupName="5b. EU-B Long")] [Browsable(false)] public int EuBLongTrendConfirmBars {get;set;}
        [NinjaScriptProperty][Display(Name="Enable ADX Filter",Order=35,GroupName="5b. EU-B Long")] [Browsable(false)] public bool EuBLongEnableAdxFilter {get;set;}
        [NinjaScriptProperty][Range(1,100)][Display(Name="ADX Period",Order=36,GroupName="5b. EU-B Long")] [Browsable(false)] public int EuBLongAdxPeriod {get;set;}
        [NinjaScriptProperty][Range(0.0,100.0)][Display(Name="ADX Min Level",Order=37,GroupName="5b. EU-B Long")] [Browsable(false)] public double EuBLongAdxMinLevel {get;set;}
        [NinjaScriptProperty][Range(0.0,100.0)][Display(Name="ADX Max Level",Order=38,GroupName="5b. EU-B Long")] [Browsable(false)] public double EuBLongAdxMaxLevel {get;set;}
        [NinjaScriptProperty][Range(0.0,100.0)][Display(Name="ADX Peak Drawdown",Order=39,GroupName="5b. EU-B Long")] [Browsable(false)] public double EuBLongAdxPeakDrawdown {get;set;}
        [NinjaScriptProperty][Range(0.01,100)][Display(Name="Candle Multiplier",Order=4,GroupName="5c. EU-B Short")] [Browsable(false)] public double EuBShortCandleMultiplier {get;set;}
        [NinjaScriptProperty][Range(0,9999)][Display(Name="Max TP Ticks",Order=5,GroupName="5c. EU-B Short")] [Browsable(false)] public int EuBShortMaxTakeProfitTicks {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="SL Extra Ticks",Order=6,GroupName="5c. EU-B Short")] [Browsable(false)] public int EuBShortSlExtraTicks {get;set;}
        [NinjaScriptProperty][Range(0,9999)][Display(Name="Max SL Ticks",Order=7,GroupName="5c. EU-B Short")] [Browsable(false)] public int EuBShortMaxSlTicks {get;set;}
        [NinjaScriptProperty][Range(0,10)][Display(Name="Max SL:TP Ratio",Order=8,GroupName="5c. EU-B Short")] [Browsable(false)] public double EuBShortMaxSlToTpRatio {get;set;}
        [NinjaScriptProperty][Display(Name="Use Prior Candle SL",Order=9,GroupName="5c. EU-B Short")] [Browsable(false)] public bool EuBShortUsePriorCandleSl {get;set;}
        [NinjaScriptProperty][Display(Name="SL At MA",Order=10,GroupName="5c. EU-B Short")] [Browsable(false)] public bool EuBShortSlAtMa {get;set;}
        [NinjaScriptProperty][Display(Name="Move SL To Entry Bar",Order=11,GroupName="5c. EU-B Short")] [Browsable(false)] public bool EuBShortMoveSlToEntryBar {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="Trail Offset Ticks",Order=12,GroupName="5c. EU-B Short")] [Browsable(false)] public int EuBShortTrailOffsetTicks {get;set;}
        [NinjaScriptProperty][Range(0,100)][Display(Name="Trail Delay Bars",Order=13,GroupName="5c. EU-B Short")] [Browsable(false)] public int EuBShortTrailDelayBars {get;set;}
        [NinjaScriptProperty][Range(0,500)][Display(Name="Trail Candle Offset",Order=14,GroupName="5c. EU-B Short")] [Browsable(false)] public int EuBShortTrailCandleOffset {get;set;}
        [NinjaScriptProperty][Display(Name="Enable Breakeven",Order=15,GroupName="5c. EU-B Short")] [Browsable(false)] public bool EuBShortEnableBreakeven {get;set;}
        [NinjaScriptProperty][Range(1,9999)][Display(Name="BE Trigger Ticks",Order=17,GroupName="5c. EU-B Short")] [Browsable(false)] public int EuBShortBreakevenTriggerTicks {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="BE Offset Ticks",Order=19,GroupName="5c. EU-B Short")] [Browsable(false)] public int EuBShortBreakevenOffsetTicks {get;set;}
        [NinjaScriptProperty][Range(0,200)][Display(Name="Max Bars In Trade",Order=20,GroupName="5c. EU-B Short")] [Browsable(false)] public int EuBShortMaxBarsInTrade {get;set;}
        [NinjaScriptProperty][Display(Name="Enable PriceOffset Trail",Order=21,GroupName="5c. EU-B Short")] [Browsable(false)] public bool EuBShortEnablePriceOffsetTrail {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="PriceOffset Reduction",Order=22,GroupName="5c. EU-B Short")] [Browsable(false)] public int EuBShortPriceOffsetReductionTicks {get;set;}
        [NinjaScriptProperty][Display(Name="Enable WMA Filter",Order=29,GroupName="5c. EU-B Short")] [Browsable(false)] public bool EuBShortEnableWmaFilter {get;set;}
        [NinjaScriptProperty][Range(1,500)][Display(Name="WMA Period",Order=30,GroupName="5c. EU-B Short")] [Browsable(false)] public int EuBShortWmaPeriod {get;set;}
        [NinjaScriptProperty][Range(0,100)][Display(Name="Min Body %",Order=32,GroupName="5c. EU-B Short")] [Browsable(false)] public double EuBShortMinBodyPct {get;set;}
        [NinjaScriptProperty][Range(0,200)][Display(Name="Trend Confirm Bars",Order=33,GroupName="5c. EU-B Short")] [Browsable(false)] public int EuBShortTrendConfirmBars {get;set;}
        [NinjaScriptProperty][Display(Name="Enable ADX Filter",Order=35,GroupName="5c. EU-B Short")] [Browsable(false)] public bool EuBShortEnableAdxFilter {get;set;}
        [NinjaScriptProperty][Range(1,100)][Display(Name="ADX Period",Order=36,GroupName="5c. EU-B Short")] [Browsable(false)] public int EuBShortAdxPeriod {get;set;}
        [NinjaScriptProperty][Range(0.0,100.0)][Display(Name="ADX Min Level",Order=37,GroupName="5c. EU-B Short")] [Browsable(false)] public double EuBShortAdxMinLevel {get;set;}
        [NinjaScriptProperty][Range(0.0,100.0)][Display(Name="ADX Max Level",Order=38,GroupName="5c. EU-B Short")] [Browsable(false)] public double EuBShortAdxMaxLevel {get;set;}
        [NinjaScriptProperty][Range(0.0,100.0)][Display(Name="ADX Peak Drawdown",Order=39,GroupName="5c. EU-B Short")] [Browsable(false)] public double EuBShortAdxPeakDrawdown {get;set;}
        // ── EU-C ────────────────────────────────────────────────────────────────
        [NinjaScriptProperty][Display(Name="Enable",Order=1,GroupName="6a. EU-C Session")] [Browsable(false)] public bool EuCEnable {get;set;}
        [NinjaScriptProperty][PropertyEditor("NinjaTrader.Gui.Tools.TimeEditorKey")][Display(Name="Trade Window Start",Order=2,GroupName="6a. EU-C Session")] [Browsable(false)] public DateTime EuCTradeWindowStart {get;set;}
        [NinjaScriptProperty][Display(Name="Enable No-New-Trades After",Order=3,GroupName="6a. EU-C Session")] [Browsable(false)] public bool EuCEnableNoNewTradesAfter {get;set;}
        [NinjaScriptProperty][PropertyEditor("NinjaTrader.Gui.Tools.TimeEditorKey")][Display(Name="No New Trades After",Order=4,GroupName="6a. EU-C Session")] [Browsable(false)] public DateTime EuCNoNewTradesAfter {get;set;}
        [NinjaScriptProperty][Display(Name="Enable Forced Close",Order=5,GroupName="6a. EU-C Session")] [Browsable(false)] public bool EuCEnableForcedClose {get;set;}
        [NinjaScriptProperty][PropertyEditor("NinjaTrader.Gui.Tools.TimeEditorKey")][Display(Name="Forced Close Time",Order=6,GroupName="6a. EU-C Session")] [Browsable(false)] public DateTime EuCForcedCloseTime {get;set;}
        [NinjaScriptProperty][Range(1,9999)][Display(Name="Max Profit Ticks",Order=10,GroupName="6a. EU-C Session")] [Browsable(false)] public int EuCMaxSessionProfitTicks {get;set;}
        [NinjaScriptProperty][Range(1,9999)][Display(Name="Max Loss Ticks",Order=11,GroupName="6a. EU-C Session")] [Browsable(false)] public int EuCMaxSessionLossTicks {get;set;}
                [NinjaScriptProperty][Range(1,20)][Display(Name="Max Trades",Order=8,GroupName="6a. EU-C Session")] [Browsable(false)] public int EuCMaxTradesPerSession {get;set;}
        [NinjaScriptProperty][Range(0,20)][Display(Name="Max Losses",Order=9,GroupName="6a. EU-C Session")] [Browsable(false)] public int EuCMaxLossesPerSession {get;set;}
        [NinjaScriptProperty][Display(Name="Require Direction Flip",Order=15,GroupName="6a. EU-C Session")] [Browsable(false)] public bool EuCRequireDirectionFlip {get;set;}
        [NinjaScriptProperty][Display(Name="Allow Same Dir After Loss",Order=16,GroupName="6a. EU-C Session")] [Browsable(false)] public bool EuCAllowSameDirectionAfterLoss {get;set;}
        [NinjaScriptProperty][Range(0,200)][Display(Name="Engulfing Exit After Bars",Order=17,GroupName="6a. EU-C Session")] [Browsable(false)] public int EuCEngulfingExitAfterBars {get;set;}
        [NinjaScriptProperty][Range(0,50)][Display(Name="Engulfing Exit Padding Ticks",Order=18,GroupName="6a. EU-C Session")] [Browsable(false)] public int EuCEngulfingExitPaddingTicks {get;set;}
        [NinjaScriptProperty][Range(0.01,100)][Display(Name="Candle Multiplier",Order=4,GroupName="6b. EU-C Long")] [Browsable(false)] public double EuCLongCandleMultiplier {get;set;}
        [NinjaScriptProperty][Range(0,9999)][Display(Name="Max TP Ticks",Order=5,GroupName="6b. EU-C Long")] [Browsable(false)] public int EuCLongMaxTakeProfitTicks {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="SL Extra Ticks",Order=6,GroupName="6b. EU-C Long")] [Browsable(false)] public int EuCLongSlExtraTicks {get;set;}
        [NinjaScriptProperty][Range(0,9999)][Display(Name="Max SL Ticks",Order=7,GroupName="6b. EU-C Long")] [Browsable(false)] public int EuCLongMaxSlTicks {get;set;}
        [NinjaScriptProperty][Range(0,10)][Display(Name="Max SL:TP Ratio",Order=8,GroupName="6b. EU-C Long")] [Browsable(false)] public double EuCLongMaxSlToTpRatio {get;set;}
        [NinjaScriptProperty][Display(Name="Use Prior Candle SL",Order=9,GroupName="6b. EU-C Long")] [Browsable(false)] public bool EuCLongUsePriorCandleSl {get;set;}
        [NinjaScriptProperty][Display(Name="SL At MA",Order=10,GroupName="6b. EU-C Long")] [Browsable(false)] public bool EuCLongSlAtMa {get;set;}
        [NinjaScriptProperty][Display(Name="Move SL To Entry Bar",Order=11,GroupName="6b. EU-C Long")] [Browsable(false)] public bool EuCLongMoveSlToEntryBar {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="Trail Offset Ticks",Order=12,GroupName="6b. EU-C Long")] [Browsable(false)] public int EuCLongTrailOffsetTicks {get;set;}
        [NinjaScriptProperty][Range(0,100)][Display(Name="Trail Delay Bars",Order=13,GroupName="6b. EU-C Long")] [Browsable(false)] public int EuCLongTrailDelayBars {get;set;}
        [NinjaScriptProperty][Range(0,500)][Display(Name="Trail Candle Offset",Order=14,GroupName="6b. EU-C Long")] [Browsable(false)] public int EuCLongTrailCandleOffset {get;set;}
        [NinjaScriptProperty][Display(Name="Enable Breakeven",Order=15,GroupName="6b. EU-C Long")] [Browsable(false)] public bool EuCLongEnableBreakeven {get;set;}
        [NinjaScriptProperty][Range(1,9999)][Display(Name="BE Trigger Ticks",Order=17,GroupName="6b. EU-C Long")] [Browsable(false)] public int EuCLongBreakevenTriggerTicks {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="BE Offset Ticks",Order=19,GroupName="6b. EU-C Long")] [Browsable(false)] public int EuCLongBreakevenOffsetTicks {get;set;}
        [NinjaScriptProperty][Range(0,200)][Display(Name="Max Bars In Trade",Order=20,GroupName="6b. EU-C Long")] [Browsable(false)] public int EuCLongMaxBarsInTrade {get;set;}
        [NinjaScriptProperty][Display(Name="Enable PriceOffset Trail",Order=21,GroupName="6b. EU-C Long")] [Browsable(false)] public bool EuCLongEnablePriceOffsetTrail {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="PriceOffset Reduction",Order=22,GroupName="6b. EU-C Long")] [Browsable(false)] public int EuCLongPriceOffsetReductionTicks {get;set;}
        [NinjaScriptProperty][Display(Name="Enable WMA Filter",Order=29,GroupName="6b. EU-C Long")] [Browsable(false)] public bool EuCLongEnableWmaFilter {get;set;}
        [NinjaScriptProperty][Range(1,500)][Display(Name="WMA Period",Order=30,GroupName="6b. EU-C Long")] [Browsable(false)] public int EuCLongWmaPeriod {get;set;}
        [NinjaScriptProperty][Range(0,100)][Display(Name="Min Body %",Order=32,GroupName="6b. EU-C Long")] [Browsable(false)] public double EuCLongMinBodyPct {get;set;}
        [NinjaScriptProperty][Range(0,200)][Display(Name="Trend Confirm Bars",Order=33,GroupName="6b. EU-C Long")] [Browsable(false)] public int EuCLongTrendConfirmBars {get;set;}
        [NinjaScriptProperty][Display(Name="Enable ADX Filter",Order=35,GroupName="6b. EU-C Long")] [Browsable(false)] public bool EuCLongEnableAdxFilter {get;set;}
        [NinjaScriptProperty][Range(1,100)][Display(Name="ADX Period",Order=36,GroupName="6b. EU-C Long")] [Browsable(false)] public int EuCLongAdxPeriod {get;set;}
        [NinjaScriptProperty][Range(0.0,100.0)][Display(Name="ADX Min Level",Order=37,GroupName="6b. EU-C Long")] [Browsable(false)] public double EuCLongAdxMinLevel {get;set;}
        [NinjaScriptProperty][Range(0.0,100.0)][Display(Name="ADX Max Level",Order=38,GroupName="6b. EU-C Long")] [Browsable(false)] public double EuCLongAdxMaxLevel {get;set;}
        [NinjaScriptProperty][Range(0.0,100.0)][Display(Name="ADX Peak Drawdown",Order=39,GroupName="6b. EU-C Long")] [Browsable(false)] public double EuCLongAdxPeakDrawdown {get;set;}
        [NinjaScriptProperty][Range(0.01,100)][Display(Name="Candle Multiplier",Order=4,GroupName="6c. EU-C Short")] [Browsable(false)] public double EuCShortCandleMultiplier {get;set;}
        [NinjaScriptProperty][Range(0,9999)][Display(Name="Max TP Ticks",Order=5,GroupName="6c. EU-C Short")] [Browsable(false)] public int EuCShortMaxTakeProfitTicks {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="SL Extra Ticks",Order=6,GroupName="6c. EU-C Short")] [Browsable(false)] public int EuCShortSlExtraTicks {get;set;}
        [NinjaScriptProperty][Range(0,9999)][Display(Name="Max SL Ticks",Order=7,GroupName="6c. EU-C Short")] [Browsable(false)] public int EuCShortMaxSlTicks {get;set;}
        [NinjaScriptProperty][Range(0,10)][Display(Name="Max SL:TP Ratio",Order=8,GroupName="6c. EU-C Short")] [Browsable(false)] public double EuCShortMaxSlToTpRatio {get;set;}
        [NinjaScriptProperty][Display(Name="Use Prior Candle SL",Order=9,GroupName="6c. EU-C Short")] [Browsable(false)] public bool EuCShortUsePriorCandleSl {get;set;}
        [NinjaScriptProperty][Display(Name="SL At MA",Order=10,GroupName="6c. EU-C Short")] [Browsable(false)] public bool EuCShortSlAtMa {get;set;}
        [NinjaScriptProperty][Display(Name="Move SL To Entry Bar",Order=11,GroupName="6c. EU-C Short")] [Browsable(false)] public bool EuCShortMoveSlToEntryBar {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="Trail Offset Ticks",Order=12,GroupName="6c. EU-C Short")] [Browsable(false)] public int EuCShortTrailOffsetTicks {get;set;}
        [NinjaScriptProperty][Range(0,100)][Display(Name="Trail Delay Bars",Order=13,GroupName="6c. EU-C Short")] [Browsable(false)] public int EuCShortTrailDelayBars {get;set;}
        [NinjaScriptProperty][Range(0,500)][Display(Name="Trail Candle Offset",Order=14,GroupName="6c. EU-C Short")] [Browsable(false)] public int EuCShortTrailCandleOffset {get;set;}
        [NinjaScriptProperty][Display(Name="Enable Breakeven",Order=15,GroupName="6c. EU-C Short")] [Browsable(false)] public bool EuCShortEnableBreakeven {get;set;}
        [NinjaScriptProperty][Range(1,9999)][Display(Name="BE Trigger Ticks",Order=17,GroupName="6c. EU-C Short")] [Browsable(false)] public int EuCShortBreakevenTriggerTicks {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="BE Offset Ticks",Order=19,GroupName="6c. EU-C Short")] [Browsable(false)] public int EuCShortBreakevenOffsetTicks {get;set;}
        [NinjaScriptProperty][Range(0,200)][Display(Name="Max Bars In Trade",Order=20,GroupName="6c. EU-C Short")] [Browsable(false)] public int EuCShortMaxBarsInTrade {get;set;}
        [NinjaScriptProperty][Display(Name="Enable PriceOffset Trail",Order=21,GroupName="6c. EU-C Short")] [Browsable(false)] public bool EuCShortEnablePriceOffsetTrail {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="PriceOffset Reduction",Order=22,GroupName="6c. EU-C Short")] [Browsable(false)] public int EuCShortPriceOffsetReductionTicks {get;set;}
        [NinjaScriptProperty][Display(Name="Enable WMA Filter",Order=29,GroupName="6c. EU-C Short")] [Browsable(false)] public bool EuCShortEnableWmaFilter {get;set;}
        [NinjaScriptProperty][Range(1,500)][Display(Name="WMA Period",Order=30,GroupName="6c. EU-C Short")] [Browsable(false)] public int EuCShortWmaPeriod {get;set;}
        [NinjaScriptProperty][Range(0,100)][Display(Name="Min Body %",Order=32,GroupName="6c. EU-C Short")] [Browsable(false)] public double EuCShortMinBodyPct {get;set;}
        [NinjaScriptProperty][Range(0,200)][Display(Name="Trend Confirm Bars",Order=33,GroupName="6c. EU-C Short")] [Browsable(false)] public int EuCShortTrendConfirmBars {get;set;}
        [NinjaScriptProperty][Display(Name="Enable ADX Filter",Order=35,GroupName="6c. EU-C Short")] [Browsable(false)] public bool EuCShortEnableAdxFilter {get;set;}
        [NinjaScriptProperty][Range(1,100)][Display(Name="ADX Period",Order=36,GroupName="6c. EU-C Short")] [Browsable(false)] public int EuCShortAdxPeriod {get;set;}
        [NinjaScriptProperty][Range(0.0,100.0)][Display(Name="ADX Min Level",Order=37,GroupName="6c. EU-C Short")] [Browsable(false)] public double EuCShortAdxMinLevel {get;set;}
        [NinjaScriptProperty][Range(0.0,100.0)][Display(Name="ADX Max Level",Order=38,GroupName="6c. EU-C Short")] [Browsable(false)] public double EuCShortAdxMaxLevel {get;set;}
        [NinjaScriptProperty][Range(0.0,100.0)][Display(Name="ADX Peak Drawdown",Order=39,GroupName="6c. EU-C Short")] [Browsable(false)] public double EuCShortAdxPeakDrawdown {get;set;}
        // ── AS-A ────────────────────────────────────────────────────────────────
        [NinjaScriptProperty][Display(Name="Enable",Order=1,GroupName="7a. AS-A Session")] [Browsable(false)] public bool AsAEnable {get;set;}
        [NinjaScriptProperty][PropertyEditor("NinjaTrader.Gui.Tools.TimeEditorKey")][Display(Name="Trade Window Start",Order=2,GroupName="7a. AS-A Session")] [Browsable(false)] public DateTime AsATradeWindowStart {get;set;}
        [NinjaScriptProperty][Display(Name="Enable No-New-Trades After",Order=3,GroupName="7a. AS-A Session")] [Browsable(false)] public bool AsAEnableNoNewTradesAfter {get;set;}
        [NinjaScriptProperty][PropertyEditor("NinjaTrader.Gui.Tools.TimeEditorKey")][Display(Name="No New Trades After",Order=4,GroupName="7a. AS-A Session")] [Browsable(false)] public DateTime AsANoNewTradesAfter {get;set;}
        [NinjaScriptProperty][Display(Name="Enable Forced Close",Order=5,GroupName="7a. AS-A Session")] [Browsable(false)] public bool AsAEnableForcedClose {get;set;}
        [NinjaScriptProperty][PropertyEditor("NinjaTrader.Gui.Tools.TimeEditorKey")][Display(Name="Forced Close Time",Order=6,GroupName="7a. AS-A Session")] [Browsable(false)] public DateTime AsAForcedCloseTime {get;set;}
        [NinjaScriptProperty][Range(1,9999)][Display(Name="Max Profit Ticks",Order=10,GroupName="7a. AS-A Session")] [Browsable(false)] public int AsAMaxSessionProfitTicks {get;set;}
        [NinjaScriptProperty][Range(1,9999)][Display(Name="Max Loss Ticks",Order=11,GroupName="7a. AS-A Session")] [Browsable(false)] public int AsAMaxSessionLossTicks {get;set;}
                [NinjaScriptProperty][Range(1,20)][Display(Name="Max Trades",Order=8,GroupName="7a. AS-A Session")] [Browsable(false)] public int AsAMaxTradesPerSession {get;set;}
        [NinjaScriptProperty][Range(0,20)][Display(Name="Max Losses",Order=9,GroupName="7a. AS-A Session")] [Browsable(false)] public int AsAMaxLossesPerSession {get;set;}
        [NinjaScriptProperty][Display(Name="Require Direction Flip",Order=15,GroupName="7a. AS-A Session")] [Browsable(false)] public bool AsARequireDirectionFlip {get;set;}
        [NinjaScriptProperty][Display(Name="Allow Same Dir After Loss",Order=16,GroupName="7a. AS-A Session")] [Browsable(false)] public bool AsAAllowSameDirectionAfterLoss {get;set;}
        [NinjaScriptProperty][Range(0,200)][Display(Name="Engulfing Exit After Bars",Order=17,GroupName="7a. AS-A Session")] [Browsable(false)] public int AsAEngulfingExitAfterBars {get;set;}
        [NinjaScriptProperty][Range(0,50)][Display(Name="Engulfing Exit Padding Ticks",Order=18,GroupName="7a. AS-A Session")] [Browsable(false)] public int AsAEngulfingExitPaddingTicks {get;set;}
        [NinjaScriptProperty][Range(0.01,100)][Display(Name="Candle Multiplier",Order=4,GroupName="7b. AS-A Long")] [Browsable(false)] public double AsALongCandleMultiplier {get;set;}
        [NinjaScriptProperty][Range(0,9999)][Display(Name="Max TP Ticks",Order=5,GroupName="7b. AS-A Long")] [Browsable(false)] public int AsALongMaxTakeProfitTicks {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="SL Extra Ticks",Order=6,GroupName="7b. AS-A Long")] [Browsable(false)] public int AsALongSlExtraTicks {get;set;}
        [NinjaScriptProperty][Range(0,9999)][Display(Name="Max SL Ticks",Order=7,GroupName="7b. AS-A Long")] [Browsable(false)] public int AsALongMaxSlTicks {get;set;}
        [NinjaScriptProperty][Range(0,10)][Display(Name="Max SL:TP Ratio",Order=8,GroupName="7b. AS-A Long")] [Browsable(false)] public double AsALongMaxSlToTpRatio {get;set;}
        [NinjaScriptProperty][Display(Name="Use Prior Candle SL",Order=9,GroupName="7b. AS-A Long")] [Browsable(false)] public bool AsALongUsePriorCandleSl {get;set;}
        [NinjaScriptProperty][Display(Name="SL At MA",Order=10,GroupName="7b. AS-A Long")] [Browsable(false)] public bool AsALongSlAtMa {get;set;}
        [NinjaScriptProperty][Display(Name="Move SL To Entry Bar",Order=11,GroupName="7b. AS-A Long")] [Browsable(false)] public bool AsALongMoveSlToEntryBar {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="Trail Offset Ticks",Order=12,GroupName="7b. AS-A Long")] [Browsable(false)] public int AsALongTrailOffsetTicks {get;set;}
        [NinjaScriptProperty][Range(0,100)][Display(Name="Trail Delay Bars",Order=13,GroupName="7b. AS-A Long")] [Browsable(false)] public int AsALongTrailDelayBars {get;set;}
        [NinjaScriptProperty][Range(0,500)][Display(Name="Trail Candle Offset",Order=14,GroupName="7b. AS-A Long")] [Browsable(false)] public int AsALongTrailCandleOffset {get;set;}
        [NinjaScriptProperty][Display(Name="Enable Breakeven",Order=15,GroupName="7b. AS-A Long")] [Browsable(false)] public bool AsALongEnableBreakeven {get;set;}
        [NinjaScriptProperty][Range(1,9999)][Display(Name="BE Trigger Ticks",Order=17,GroupName="7b. AS-A Long")] [Browsable(false)] public int AsALongBreakevenTriggerTicks {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="BE Offset Ticks",Order=19,GroupName="7b. AS-A Long")] [Browsable(false)] public int AsALongBreakevenOffsetTicks {get;set;}
        [NinjaScriptProperty][Range(0,200)][Display(Name="Max Bars In Trade",Order=20,GroupName="7b. AS-A Long")] [Browsable(false)] public int AsALongMaxBarsInTrade {get;set;}
        [NinjaScriptProperty][Display(Name="Enable PriceOffset Trail",Order=21,GroupName="7b. AS-A Long")] [Browsable(false)] public bool AsALongEnablePriceOffsetTrail {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="PriceOffset Reduction",Order=22,GroupName="7b. AS-A Long")] [Browsable(false)] public int AsALongPriceOffsetReductionTicks {get;set;}
        [NinjaScriptProperty][Display(Name="Enable WMA Filter",Order=29,GroupName="7b. AS-A Long")] [Browsable(false)] public bool AsALongEnableWmaFilter {get;set;}
        [NinjaScriptProperty][Range(1,500)][Display(Name="WMA Period",Order=30,GroupName="7b. AS-A Long")] [Browsable(false)] public int AsALongWmaPeriod {get;set;}
        [NinjaScriptProperty][Range(0,100)][Display(Name="Min Body %",Order=32,GroupName="7b. AS-A Long")] [Browsable(false)] public double AsALongMinBodyPct {get;set;}
        [NinjaScriptProperty][Range(0,200)][Display(Name="Trend Confirm Bars",Order=33,GroupName="7b. AS-A Long")] [Browsable(false)] public int AsALongTrendConfirmBars {get;set;}
        [NinjaScriptProperty][Display(Name="Enable ADX Filter",Order=35,GroupName="7b. AS-A Long")] [Browsable(false)] public bool AsALongEnableAdxFilter {get;set;}
        [NinjaScriptProperty][Range(1,100)][Display(Name="ADX Period",Order=36,GroupName="7b. AS-A Long")] [Browsable(false)] public int AsALongAdxPeriod {get;set;}
        [NinjaScriptProperty][Range(0.0,100.0)][Display(Name="ADX Min Level",Order=37,GroupName="7b. AS-A Long")] [Browsable(false)] public double AsALongAdxMinLevel {get;set;}
        [NinjaScriptProperty][Range(0.0,100.0)][Display(Name="ADX Max Level",Order=38,GroupName="7b. AS-A Long")] [Browsable(false)] public double AsALongAdxMaxLevel {get;set;}
        [NinjaScriptProperty][Range(0.0,100.0)][Display(Name="ADX Peak Drawdown",Order=39,GroupName="7b. AS-A Long")] [Browsable(false)] public double AsALongAdxPeakDrawdown {get;set;}
        [NinjaScriptProperty][Range(0.01,100)][Display(Name="Candle Multiplier",Order=4,GroupName="7c. AS-A Short")] [Browsable(false)] public double AsAShortCandleMultiplier {get;set;}
        [NinjaScriptProperty][Range(0,9999)][Display(Name="Max TP Ticks",Order=5,GroupName="7c. AS-A Short")] [Browsable(false)] public int AsAShortMaxTakeProfitTicks {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="SL Extra Ticks",Order=6,GroupName="7c. AS-A Short")] [Browsable(false)] public int AsAShortSlExtraTicks {get;set;}
        [NinjaScriptProperty][Range(0,9999)][Display(Name="Max SL Ticks",Order=7,GroupName="7c. AS-A Short")] [Browsable(false)] public int AsAShortMaxSlTicks {get;set;}
        [NinjaScriptProperty][Range(0,10)][Display(Name="Max SL:TP Ratio",Order=8,GroupName="7c. AS-A Short")] [Browsable(false)] public double AsAShortMaxSlToTpRatio {get;set;}
        [NinjaScriptProperty][Display(Name="Use Prior Candle SL",Order=9,GroupName="7c. AS-A Short")] [Browsable(false)] public bool AsAShortUsePriorCandleSl {get;set;}
        [NinjaScriptProperty][Display(Name="SL At MA",Order=10,GroupName="7c. AS-A Short")] [Browsable(false)] public bool AsAShortSlAtMa {get;set;}
        [NinjaScriptProperty][Display(Name="Move SL To Entry Bar",Order=11,GroupName="7c. AS-A Short")] [Browsable(false)] public bool AsAShortMoveSlToEntryBar {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="Trail Offset Ticks",Order=12,GroupName="7c. AS-A Short")] [Browsable(false)] public int AsAShortTrailOffsetTicks {get;set;}
        [NinjaScriptProperty][Range(0,100)][Display(Name="Trail Delay Bars",Order=13,GroupName="7c. AS-A Short")] [Browsable(false)] public int AsAShortTrailDelayBars {get;set;}
        [NinjaScriptProperty][Range(0,500)][Display(Name="Trail Candle Offset",Order=14,GroupName="7c. AS-A Short")] [Browsable(false)] public int AsAShortTrailCandleOffset {get;set;}
        [NinjaScriptProperty][Display(Name="Enable Breakeven",Order=15,GroupName="7c. AS-A Short")] [Browsable(false)] public bool AsAShortEnableBreakeven {get;set;}
        [NinjaScriptProperty][Range(1,9999)][Display(Name="BE Trigger Ticks",Order=17,GroupName="7c. AS-A Short")] [Browsable(false)] public int AsAShortBreakevenTriggerTicks {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="BE Offset Ticks",Order=19,GroupName="7c. AS-A Short")] [Browsable(false)] public int AsAShortBreakevenOffsetTicks {get;set;}
        [NinjaScriptProperty][Range(0,200)][Display(Name="Max Bars In Trade",Order=20,GroupName="7c. AS-A Short")] [Browsable(false)] public int AsAShortMaxBarsInTrade {get;set;}
        [NinjaScriptProperty][Display(Name="Enable PriceOffset Trail",Order=21,GroupName="7c. AS-A Short")] [Browsable(false)] public bool AsAShortEnablePriceOffsetTrail {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="PriceOffset Reduction",Order=22,GroupName="7c. AS-A Short")] [Browsable(false)] public int AsAShortPriceOffsetReductionTicks {get;set;}
        [NinjaScriptProperty][Display(Name="Enable WMA Filter",Order=29,GroupName="7c. AS-A Short")] [Browsable(false)] public bool AsAShortEnableWmaFilter {get;set;}
        [NinjaScriptProperty][Range(1,500)][Display(Name="WMA Period",Order=30,GroupName="7c. AS-A Short")] [Browsable(false)] public int AsAShortWmaPeriod {get;set;}
        [NinjaScriptProperty][Range(0,100)][Display(Name="Min Body %",Order=32,GroupName="7c. AS-A Short")] [Browsable(false)] public double AsAShortMinBodyPct {get;set;}
        [NinjaScriptProperty][Range(0,200)][Display(Name="Trend Confirm Bars",Order=33,GroupName="7c. AS-A Short")] [Browsable(false)] public int AsAShortTrendConfirmBars {get;set;}
        [NinjaScriptProperty][Display(Name="Enable ADX Filter",Order=35,GroupName="7c. AS-A Short")] [Browsable(false)] public bool AsAShortEnableAdxFilter {get;set;}
        [NinjaScriptProperty][Range(1,100)][Display(Name="ADX Period",Order=36,GroupName="7c. AS-A Short")] [Browsable(false)] public int AsAShortAdxPeriod {get;set;}
        [NinjaScriptProperty][Range(0.0,100.0)][Display(Name="ADX Min Level",Order=37,GroupName="7c. AS-A Short")] [Browsable(false)] public double AsAShortAdxMinLevel {get;set;}
        [NinjaScriptProperty][Range(0.0,100.0)][Display(Name="ADX Max Level",Order=38,GroupName="7c. AS-A Short")] [Browsable(false)] public double AsAShortAdxMaxLevel {get;set;}
        [NinjaScriptProperty][Range(0.0,100.0)][Display(Name="ADX Peak Drawdown",Order=39,GroupName="7c. AS-A Short")] [Browsable(false)] public double AsAShortAdxPeakDrawdown {get;set;}
        // ── AS-B ────────────────────────────────────────────────────────────────
        [NinjaScriptProperty][Display(Name="Enable",Order=1,GroupName="8a. AS-B Session")] [Browsable(false)] public bool AsBEnable {get;set;}
        [NinjaScriptProperty][PropertyEditor("NinjaTrader.Gui.Tools.TimeEditorKey")][Display(Name="Trade Window Start",Order=2,GroupName="8a. AS-B Session")] [Browsable(false)] public DateTime AsBTradeWindowStart {get;set;}
        [NinjaScriptProperty][Display(Name="Enable No-New-Trades After",Order=3,GroupName="8a. AS-B Session")] [Browsable(false)] public bool AsBEnableNoNewTradesAfter {get;set;}
        [NinjaScriptProperty][PropertyEditor("NinjaTrader.Gui.Tools.TimeEditorKey")][Display(Name="No New Trades After",Order=4,GroupName="8a. AS-B Session")] [Browsable(false)] public DateTime AsBNoNewTradesAfter {get;set;}
        [NinjaScriptProperty][Display(Name="Enable Forced Close",Order=5,GroupName="8a. AS-B Session")] [Browsable(false)] public bool AsBEnableForcedClose {get;set;}
        [NinjaScriptProperty][PropertyEditor("NinjaTrader.Gui.Tools.TimeEditorKey")][Display(Name="Forced Close Time",Order=6,GroupName="8a. AS-B Session")] [Browsable(false)] public DateTime AsBForcedCloseTime {get;set;}
        [NinjaScriptProperty][Range(1,9999)][Display(Name="Max Profit Ticks",Order=10,GroupName="8a. AS-B Session")] [Browsable(false)] public int AsBMaxSessionProfitTicks {get;set;}
        [NinjaScriptProperty][Range(1,9999)][Display(Name="Max Loss Ticks",Order=11,GroupName="8a. AS-B Session")] [Browsable(false)] public int AsBMaxSessionLossTicks {get;set;}
                [NinjaScriptProperty][Range(1,20)][Display(Name="Max Trades",Order=8,GroupName="8a. AS-B Session")] [Browsable(false)] public int AsBMaxTradesPerSession {get;set;}
        [NinjaScriptProperty][Range(0,20)][Display(Name="Max Losses",Order=9,GroupName="8a. AS-B Session")] [Browsable(false)] public int AsBMaxLossesPerSession {get;set;}
        [NinjaScriptProperty][Display(Name="Require Direction Flip",Order=15,GroupName="8a. AS-B Session")] [Browsable(false)] public bool AsBRequireDirectionFlip {get;set;}
        [NinjaScriptProperty][Display(Name="Allow Same Dir After Loss",Order=16,GroupName="8a. AS-B Session")] [Browsable(false)] public bool AsBAllowSameDirectionAfterLoss {get;set;}
        [NinjaScriptProperty][Range(0,200)][Display(Name="Engulfing Exit After Bars",Order=17,GroupName="8a. AS-B Session")] [Browsable(false)] public int AsBEngulfingExitAfterBars {get;set;}
        [NinjaScriptProperty][Range(0,50)][Display(Name="Engulfing Exit Padding Ticks",Order=18,GroupName="8a. AS-B Session")] [Browsable(false)] public int AsBEngulfingExitPaddingTicks {get;set;}
        [NinjaScriptProperty][Range(0.01,100)][Display(Name="Candle Multiplier",Order=4,GroupName="8b. AS-B Long")] [Browsable(false)] public double AsBLongCandleMultiplier {get;set;}
        [NinjaScriptProperty][Range(0,9999)][Display(Name="Max TP Ticks",Order=5,GroupName="8b. AS-B Long")] [Browsable(false)] public int AsBLongMaxTakeProfitTicks {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="SL Extra Ticks",Order=6,GroupName="8b. AS-B Long")] [Browsable(false)] public int AsBLongSlExtraTicks {get;set;}
        [NinjaScriptProperty][Range(0,9999)][Display(Name="Max SL Ticks",Order=7,GroupName="8b. AS-B Long")] [Browsable(false)] public int AsBLongMaxSlTicks {get;set;}
        [NinjaScriptProperty][Range(0,10)][Display(Name="Max SL:TP Ratio",Order=8,GroupName="8b. AS-B Long")] [Browsable(false)] public double AsBLongMaxSlToTpRatio {get;set;}
        [NinjaScriptProperty][Display(Name="Use Prior Candle SL",Order=9,GroupName="8b. AS-B Long")] [Browsable(false)] public bool AsBLongUsePriorCandleSl {get;set;}
        [NinjaScriptProperty][Display(Name="SL At MA",Order=10,GroupName="8b. AS-B Long")] [Browsable(false)] public bool AsBLongSlAtMa {get;set;}
        [NinjaScriptProperty][Display(Name="Move SL To Entry Bar",Order=11,GroupName="8b. AS-B Long")] [Browsable(false)] public bool AsBLongMoveSlToEntryBar {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="Trail Offset Ticks",Order=12,GroupName="8b. AS-B Long")] [Browsable(false)] public int AsBLongTrailOffsetTicks {get;set;}
        [NinjaScriptProperty][Range(0,100)][Display(Name="Trail Delay Bars",Order=13,GroupName="8b. AS-B Long")] [Browsable(false)] public int AsBLongTrailDelayBars {get;set;}
        [NinjaScriptProperty][Range(0,500)][Display(Name="Trail Candle Offset",Order=14,GroupName="8b. AS-B Long")] [Browsable(false)] public int AsBLongTrailCandleOffset {get;set;}
        [NinjaScriptProperty][Display(Name="Enable Breakeven",Order=15,GroupName="8b. AS-B Long")] [Browsable(false)] public bool AsBLongEnableBreakeven {get;set;}
        [NinjaScriptProperty][Range(1,9999)][Display(Name="BE Trigger Ticks",Order=17,GroupName="8b. AS-B Long")] [Browsable(false)] public int AsBLongBreakevenTriggerTicks {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="BE Offset Ticks",Order=19,GroupName="8b. AS-B Long")] [Browsable(false)] public int AsBLongBreakevenOffsetTicks {get;set;}
        [NinjaScriptProperty][Range(0,200)][Display(Name="Max Bars In Trade",Order=20,GroupName="8b. AS-B Long")] [Browsable(false)] public int AsBLongMaxBarsInTrade {get;set;}
        [NinjaScriptProperty][Display(Name="Enable PriceOffset Trail",Order=21,GroupName="8b. AS-B Long")] [Browsable(false)] public bool AsBLongEnablePriceOffsetTrail {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="PriceOffset Reduction",Order=22,GroupName="8b. AS-B Long")] [Browsable(false)] public int AsBLongPriceOffsetReductionTicks {get;set;}
        [NinjaScriptProperty][Display(Name="Enable WMA Filter",Order=29,GroupName="8b. AS-B Long")] [Browsable(false)] public bool AsBLongEnableWmaFilter {get;set;}
        [NinjaScriptProperty][Range(1,500)][Display(Name="WMA Period",Order=30,GroupName="8b. AS-B Long")] [Browsable(false)] public int AsBLongWmaPeriod {get;set;}
        [NinjaScriptProperty][Range(0,100)][Display(Name="Min Body %",Order=32,GroupName="8b. AS-B Long")] [Browsable(false)] public double AsBLongMinBodyPct {get;set;}
        [NinjaScriptProperty][Range(0,200)][Display(Name="Trend Confirm Bars",Order=33,GroupName="8b. AS-B Long")] [Browsable(false)] public int AsBLongTrendConfirmBars {get;set;}
        [NinjaScriptProperty][Display(Name="Enable ADX Filter",Order=35,GroupName="8b. AS-B Long")] [Browsable(false)] public bool AsBLongEnableAdxFilter {get;set;}
        [NinjaScriptProperty][Range(1,100)][Display(Name="ADX Period",Order=36,GroupName="8b. AS-B Long")] [Browsable(false)] public int AsBLongAdxPeriod {get;set;}
        [NinjaScriptProperty][Range(0.0,100.0)][Display(Name="ADX Min Level",Order=37,GroupName="8b. AS-B Long")] [Browsable(false)] public double AsBLongAdxMinLevel {get;set;}
        [NinjaScriptProperty][Range(0.0,100.0)][Display(Name="ADX Max Level",Order=38,GroupName="8b. AS-B Long")] [Browsable(false)] public double AsBLongAdxMaxLevel {get;set;}
        [NinjaScriptProperty][Range(0.0,100.0)][Display(Name="ADX Peak Drawdown",Order=39,GroupName="8b. AS-B Long")] [Browsable(false)] public double AsBLongAdxPeakDrawdown {get;set;}
        [NinjaScriptProperty][Range(0.01,100)][Display(Name="Candle Multiplier",Order=4,GroupName="8c. AS-B Short")] [Browsable(false)] public double AsBShortCandleMultiplier {get;set;}
        [NinjaScriptProperty][Range(0,9999)][Display(Name="Max TP Ticks",Order=5,GroupName="8c. AS-B Short")] [Browsable(false)] public int AsBShortMaxTakeProfitTicks {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="SL Extra Ticks",Order=6,GroupName="8c. AS-B Short")] [Browsable(false)] public int AsBShortSlExtraTicks {get;set;}
        [NinjaScriptProperty][Range(0,9999)][Display(Name="Max SL Ticks",Order=7,GroupName="8c. AS-B Short")] [Browsable(false)] public int AsBShortMaxSlTicks {get;set;}
        [NinjaScriptProperty][Range(0,10)][Display(Name="Max SL:TP Ratio",Order=8,GroupName="8c. AS-B Short")] [Browsable(false)] public double AsBShortMaxSlToTpRatio {get;set;}
        [NinjaScriptProperty][Display(Name="Use Prior Candle SL",Order=9,GroupName="8c. AS-B Short")] [Browsable(false)] public bool AsBShortUsePriorCandleSl {get;set;}
        [NinjaScriptProperty][Display(Name="SL At MA",Order=10,GroupName="8c. AS-B Short")] [Browsable(false)] public bool AsBShortSlAtMa {get;set;}
        [NinjaScriptProperty][Display(Name="Move SL To Entry Bar",Order=11,GroupName="8c. AS-B Short")] [Browsable(false)] public bool AsBShortMoveSlToEntryBar {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="Trail Offset Ticks",Order=12,GroupName="8c. AS-B Short")] [Browsable(false)] public int AsBShortTrailOffsetTicks {get;set;}
        [NinjaScriptProperty][Range(0,100)][Display(Name="Trail Delay Bars",Order=13,GroupName="8c. AS-B Short")] [Browsable(false)] public int AsBShortTrailDelayBars {get;set;}
        [NinjaScriptProperty][Range(0,500)][Display(Name="Trail Candle Offset",Order=14,GroupName="8c. AS-B Short")] [Browsable(false)] public int AsBShortTrailCandleOffset {get;set;}
        [NinjaScriptProperty][Display(Name="Enable Breakeven",Order=15,GroupName="8c. AS-B Short")] [Browsable(false)] public bool AsBShortEnableBreakeven {get;set;}
        [NinjaScriptProperty][Range(1,9999)][Display(Name="BE Trigger Ticks",Order=17,GroupName="8c. AS-B Short")] [Browsable(false)] public int AsBShortBreakevenTriggerTicks {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="BE Offset Ticks",Order=19,GroupName="8c. AS-B Short")] [Browsable(false)] public int AsBShortBreakevenOffsetTicks {get;set;}
        [NinjaScriptProperty][Range(0,200)][Display(Name="Max Bars In Trade",Order=20,GroupName="8c. AS-B Short")] [Browsable(false)] public int AsBShortMaxBarsInTrade {get;set;}
        [NinjaScriptProperty][Display(Name="Enable PriceOffset Trail",Order=21,GroupName="8c. AS-B Short")] [Browsable(false)] public bool AsBShortEnablePriceOffsetTrail {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="PriceOffset Reduction",Order=22,GroupName="8c. AS-B Short")] [Browsable(false)] public int AsBShortPriceOffsetReductionTicks {get;set;}
        [NinjaScriptProperty][Display(Name="Enable WMA Filter",Order=29,GroupName="8c. AS-B Short")] [Browsable(false)] public bool AsBShortEnableWmaFilter {get;set;}
        [NinjaScriptProperty][Range(1,500)][Display(Name="WMA Period",Order=30,GroupName="8c. AS-B Short")] [Browsable(false)] public int AsBShortWmaPeriod {get;set;}
        [NinjaScriptProperty][Range(0,100)][Display(Name="Min Body %",Order=32,GroupName="8c. AS-B Short")] [Browsable(false)] public double AsBShortMinBodyPct {get;set;}
        [NinjaScriptProperty][Range(0,200)][Display(Name="Trend Confirm Bars",Order=33,GroupName="8c. AS-B Short")] [Browsable(false)] public int AsBShortTrendConfirmBars {get;set;}
        [NinjaScriptProperty][Display(Name="Enable ADX Filter",Order=35,GroupName="8c. AS-B Short")] [Browsable(false)] public bool AsBShortEnableAdxFilter {get;set;}
        [NinjaScriptProperty][Range(1,100)][Display(Name="ADX Period",Order=36,GroupName="8c. AS-B Short")] [Browsable(false)] public int AsBShortAdxPeriod {get;set;}
        [NinjaScriptProperty][Range(0.0,100.0)][Display(Name="ADX Min Level",Order=37,GroupName="8c. AS-B Short")] [Browsable(false)] public double AsBShortAdxMinLevel {get;set;}
        [NinjaScriptProperty][Range(0.0,100.0)][Display(Name="ADX Max Level",Order=38,GroupName="8c. AS-B Short")] [Browsable(false)] public double AsBShortAdxMaxLevel {get;set;}
        [NinjaScriptProperty][Range(0.0,100.0)][Display(Name="ADX Peak Drawdown",Order=39,GroupName="8c. AS-B Short")] [Browsable(false)] public double AsBShortAdxPeakDrawdown {get;set;}
        // ── AS-C ────────────────────────────────────────────────────────────────
        [NinjaScriptProperty][Display(Name="Enable",Order=1,GroupName="9a. AS-C Session")] [Browsable(false)] public bool AsCEnable {get;set;}
        [NinjaScriptProperty][PropertyEditor("NinjaTrader.Gui.Tools.TimeEditorKey")][Display(Name="Trade Window Start",Order=2,GroupName="9a. AS-C Session")] [Browsable(false)] public DateTime AsCTradeWindowStart {get;set;}
        [NinjaScriptProperty][Display(Name="Enable No-New-Trades After",Order=3,GroupName="9a. AS-C Session")] [Browsable(false)] public bool AsCEnableNoNewTradesAfter {get;set;}
        [NinjaScriptProperty][PropertyEditor("NinjaTrader.Gui.Tools.TimeEditorKey")][Display(Name="No New Trades After",Order=4,GroupName="9a. AS-C Session")] [Browsable(false)] public DateTime AsCNoNewTradesAfter {get;set;}
        [NinjaScriptProperty][Display(Name="Enable Forced Close",Order=5,GroupName="9a. AS-C Session")] [Browsable(false)] public bool AsCEnableForcedClose {get;set;}
        [NinjaScriptProperty][PropertyEditor("NinjaTrader.Gui.Tools.TimeEditorKey")][Display(Name="Forced Close Time",Order=6,GroupName="9a. AS-C Session")] [Browsable(false)] public DateTime AsCForcedCloseTime {get;set;}
        [NinjaScriptProperty][Range(1,9999)][Display(Name="Max Profit Ticks",Order=10,GroupName="9a. AS-C Session")] [Browsable(false)] public int AsCMaxSessionProfitTicks {get;set;}
        [NinjaScriptProperty][Range(1,9999)][Display(Name="Max Loss Ticks",Order=11,GroupName="9a. AS-C Session")] [Browsable(false)] public int AsCMaxSessionLossTicks {get;set;}
                [NinjaScriptProperty][Range(1,20)][Display(Name="Max Trades",Order=8,GroupName="9a. AS-C Session")] [Browsable(false)] public int AsCMaxTradesPerSession {get;set;}
        [NinjaScriptProperty][Range(0,20)][Display(Name="Max Losses",Order=9,GroupName="9a. AS-C Session")] [Browsable(false)] public int AsCMaxLossesPerSession {get;set;}
        [NinjaScriptProperty][Display(Name="Require Direction Flip",Order=15,GroupName="9a. AS-C Session")] [Browsable(false)] public bool AsCRequireDirectionFlip {get;set;}
        [NinjaScriptProperty][Display(Name="Allow Same Dir After Loss",Order=16,GroupName="9a. AS-C Session")] [Browsable(false)] public bool AsCAllowSameDirectionAfterLoss {get;set;}
        [NinjaScriptProperty][Range(0,200)][Display(Name="Engulfing Exit After Bars",Order=17,GroupName="9a. AS-C Session")] [Browsable(false)] public int AsCEngulfingExitAfterBars {get;set;}
        [NinjaScriptProperty][Range(0,50)][Display(Name="Engulfing Exit Padding Ticks",Order=18,GroupName="9a. AS-C Session")] [Browsable(false)] public int AsCEngulfingExitPaddingTicks {get;set;}
        [NinjaScriptProperty][Range(0.01,100)][Display(Name="Candle Multiplier",Order=4,GroupName="9b. AS-C Long")] [Browsable(false)] public double AsCLongCandleMultiplier {get;set;}
        [NinjaScriptProperty][Range(0,9999)][Display(Name="Max TP Ticks",Order=5,GroupName="9b. AS-C Long")] [Browsable(false)] public int AsCLongMaxTakeProfitTicks {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="SL Extra Ticks",Order=6,GroupName="9b. AS-C Long")] [Browsable(false)] public int AsCLongSlExtraTicks {get;set;}
        [NinjaScriptProperty][Range(0,9999)][Display(Name="Max SL Ticks",Order=7,GroupName="9b. AS-C Long")] [Browsable(false)] public int AsCLongMaxSlTicks {get;set;}
        [NinjaScriptProperty][Range(0,10)][Display(Name="Max SL:TP Ratio",Order=8,GroupName="9b. AS-C Long")] [Browsable(false)] public double AsCLongMaxSlToTpRatio {get;set;}
        [NinjaScriptProperty][Display(Name="Use Prior Candle SL",Order=9,GroupName="9b. AS-C Long")] [Browsable(false)] public bool AsCLongUsePriorCandleSl {get;set;}
        [NinjaScriptProperty][Display(Name="SL At MA",Order=10,GroupName="9b. AS-C Long")] [Browsable(false)] public bool AsCLongSlAtMa {get;set;}
        [NinjaScriptProperty][Display(Name="Move SL To Entry Bar",Order=11,GroupName="9b. AS-C Long")] [Browsable(false)] public bool AsCLongMoveSlToEntryBar {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="Trail Offset Ticks",Order=12,GroupName="9b. AS-C Long")] [Browsable(false)] public int AsCLongTrailOffsetTicks {get;set;}
        [NinjaScriptProperty][Range(0,100)][Display(Name="Trail Delay Bars",Order=13,GroupName="9b. AS-C Long")] [Browsable(false)] public int AsCLongTrailDelayBars {get;set;}
        [NinjaScriptProperty][Range(0,500)][Display(Name="Trail Candle Offset",Order=14,GroupName="9b. AS-C Long")] [Browsable(false)] public int AsCLongTrailCandleOffset {get;set;}
        [NinjaScriptProperty][Display(Name="Enable Breakeven",Order=15,GroupName="9b. AS-C Long")] [Browsable(false)] public bool AsCLongEnableBreakeven {get;set;}
        [NinjaScriptProperty][Range(1,9999)][Display(Name="BE Trigger Ticks",Order=17,GroupName="9b. AS-C Long")] [Browsable(false)] public int AsCLongBreakevenTriggerTicks {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="BE Offset Ticks",Order=19,GroupName="9b. AS-C Long")] [Browsable(false)] public int AsCLongBreakevenOffsetTicks {get;set;}
        [NinjaScriptProperty][Range(0,200)][Display(Name="Max Bars In Trade",Order=20,GroupName="9b. AS-C Long")] [Browsable(false)] public int AsCLongMaxBarsInTrade {get;set;}
        [NinjaScriptProperty][Display(Name="Enable PriceOffset Trail",Order=21,GroupName="9b. AS-C Long")] [Browsable(false)] public bool AsCLongEnablePriceOffsetTrail {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="PriceOffset Reduction",Order=22,GroupName="9b. AS-C Long")] [Browsable(false)] public int AsCLongPriceOffsetReductionTicks {get;set;}
        [NinjaScriptProperty][Display(Name="Enable WMA Filter",Order=29,GroupName="9b. AS-C Long")] [Browsable(false)] public bool AsCLongEnableWmaFilter {get;set;}
        [NinjaScriptProperty][Range(1,500)][Display(Name="WMA Period",Order=30,GroupName="9b. AS-C Long")] [Browsable(false)] public int AsCLongWmaPeriod {get;set;}
        [NinjaScriptProperty][Range(0,100)][Display(Name="Min Body %",Order=32,GroupName="9b. AS-C Long")] [Browsable(false)] public double AsCLongMinBodyPct {get;set;}
        [NinjaScriptProperty][Range(0,200)][Display(Name="Trend Confirm Bars",Order=33,GroupName="9b. AS-C Long")] [Browsable(false)] public int AsCLongTrendConfirmBars {get;set;}
        [NinjaScriptProperty][Display(Name="Enable ADX Filter",Order=35,GroupName="9b. AS-C Long")] [Browsable(false)] public bool AsCLongEnableAdxFilter {get;set;}
        [NinjaScriptProperty][Range(1,100)][Display(Name="ADX Period",Order=36,GroupName="9b. AS-C Long")] [Browsable(false)] public int AsCLongAdxPeriod {get;set;}
        [NinjaScriptProperty][Range(0.0,100.0)][Display(Name="ADX Min Level",Order=37,GroupName="9b. AS-C Long")] [Browsable(false)] public double AsCLongAdxMinLevel {get;set;}
        [NinjaScriptProperty][Range(0.0,100.0)][Display(Name="ADX Max Level",Order=38,GroupName="9b. AS-C Long")] [Browsable(false)] public double AsCLongAdxMaxLevel {get;set;}
        [NinjaScriptProperty][Range(0.0,100.0)][Display(Name="ADX Peak Drawdown",Order=39,GroupName="9b. AS-C Long")] [Browsable(false)] public double AsCLongAdxPeakDrawdown {get;set;}
        [NinjaScriptProperty][Range(0.01,100)][Display(Name="Candle Multiplier",Order=4,GroupName="9c. AS-C Short")] [Browsable(false)] public double AsCShortCandleMultiplier {get;set;}
        [NinjaScriptProperty][Range(0,9999)][Display(Name="Max TP Ticks",Order=5,GroupName="9c. AS-C Short")] [Browsable(false)] public int AsCShortMaxTakeProfitTicks {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="SL Extra Ticks",Order=6,GroupName="9c. AS-C Short")] [Browsable(false)] public int AsCShortSlExtraTicks {get;set;}
        [NinjaScriptProperty][Range(0,9999)][Display(Name="Max SL Ticks",Order=7,GroupName="9c. AS-C Short")] [Browsable(false)] public int AsCShortMaxSlTicks {get;set;}
        [NinjaScriptProperty][Range(0,10)][Display(Name="Max SL:TP Ratio",Order=8,GroupName="9c. AS-C Short")] [Browsable(false)] public double AsCShortMaxSlToTpRatio {get;set;}
        [NinjaScriptProperty][Display(Name="Use Prior Candle SL",Order=9,GroupName="9c. AS-C Short")] [Browsable(false)] public bool AsCShortUsePriorCandleSl {get;set;}
        [NinjaScriptProperty][Display(Name="SL At MA",Order=10,GroupName="9c. AS-C Short")] [Browsable(false)] public bool AsCShortSlAtMa {get;set;}
        [NinjaScriptProperty][Display(Name="Move SL To Entry Bar",Order=11,GroupName="9c. AS-C Short")] [Browsable(false)] public bool AsCShortMoveSlToEntryBar {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="Trail Offset Ticks",Order=12,GroupName="9c. AS-C Short")] [Browsable(false)] public int AsCShortTrailOffsetTicks {get;set;}
        [NinjaScriptProperty][Range(0,100)][Display(Name="Trail Delay Bars",Order=13,GroupName="9c. AS-C Short")] [Browsable(false)] public int AsCShortTrailDelayBars {get;set;}
        [NinjaScriptProperty][Range(0,500)][Display(Name="Trail Candle Offset",Order=14,GroupName="9c. AS-C Short")] [Browsable(false)] public int AsCShortTrailCandleOffset {get;set;}
        [NinjaScriptProperty][Display(Name="Enable Breakeven",Order=15,GroupName="9c. AS-C Short")] [Browsable(false)] public bool AsCShortEnableBreakeven {get;set;}
        [NinjaScriptProperty][Range(1,9999)][Display(Name="BE Trigger Ticks",Order=17,GroupName="9c. AS-C Short")] [Browsable(false)] public int AsCShortBreakevenTriggerTicks {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="BE Offset Ticks",Order=19,GroupName="9c. AS-C Short")] [Browsable(false)] public int AsCShortBreakevenOffsetTicks {get;set;}
        [NinjaScriptProperty][Range(0,200)][Display(Name="Max Bars In Trade",Order=20,GroupName="9c. AS-C Short")] [Browsable(false)] public int AsCShortMaxBarsInTrade {get;set;}
        [NinjaScriptProperty][Display(Name="Enable PriceOffset Trail",Order=21,GroupName="9c. AS-C Short")] [Browsable(false)] public bool AsCShortEnablePriceOffsetTrail {get;set;}
        [NinjaScriptProperty][Range(0,999)][Display(Name="PriceOffset Reduction",Order=22,GroupName="9c. AS-C Short")] [Browsable(false)] public int AsCShortPriceOffsetReductionTicks {get;set;}
        [NinjaScriptProperty][Display(Name="Enable WMA Filter",Order=29,GroupName="9c. AS-C Short")] [Browsable(false)] public bool AsCShortEnableWmaFilter {get;set;}
        [NinjaScriptProperty][Range(1,500)][Display(Name="WMA Period",Order=30,GroupName="9c. AS-C Short")] [Browsable(false)] public int AsCShortWmaPeriod {get;set;}
        [NinjaScriptProperty][Range(0,100)][Display(Name="Min Body %",Order=32,GroupName="9c. AS-C Short")] [Browsable(false)] public double AsCShortMinBodyPct {get;set;}
        [NinjaScriptProperty][Range(0,200)][Display(Name="Trend Confirm Bars",Order=33,GroupName="9c. AS-C Short")] [Browsable(false)] public int AsCShortTrendConfirmBars {get;set;}
        [NinjaScriptProperty][Display(Name="Enable ADX Filter",Order=35,GroupName="9c. AS-C Short")] [Browsable(false)] public bool AsCShortEnableAdxFilter {get;set;}
        [NinjaScriptProperty][Range(1,100)][Display(Name="ADX Period",Order=36,GroupName="9c. AS-C Short")] [Browsable(false)] public int AsCShortAdxPeriod {get;set;}
        [NinjaScriptProperty][Range(0.0,100.0)][Display(Name="ADX Min Level",Order=37,GroupName="9c. AS-C Short")] [Browsable(false)] public double AsCShortAdxMinLevel {get;set;}
        [NinjaScriptProperty][Range(0.0,100.0)][Display(Name="ADX Max Level",Order=38,GroupName="9c. AS-C Short")] [Browsable(false)] public double AsCShortAdxMaxLevel {get;set;}
        [NinjaScriptProperty][Range(0.0,100.0)][Display(Name="ADX Peak Drawdown",Order=39,GroupName="9c. AS-C Short")] [Browsable(false)] public double AsCShortAdxPeakDrawdown {get;set;}
        ...
        // ── Execution ──────────────────────────────────────────────────────
        [NinjaScriptProperty][Display(Name="Entry Confirmation",Order=1,GroupName="0. Common - Execution")] public bool RequireEntryConfirmation {get;set;}
        // ── Skip / News time windows ───────────────────────────────────────
        [NinjaScriptProperty][Browsable(false)][Display(Name="Enable Skip Time Window",Order=3,GroupName="0. Common - Time Settings")] public bool EnableSkipTime {get;set;}
        [NinjaScriptProperty][Browsable(false)][Display(Name="Skip Time Start",Order=4,GroupName="0. Common - Time Settings")] public string SkipTimeStart {get;set;}
        [NinjaScriptProperty][Browsable(false)][Display(Name="Skip Time End",Order=5,GroupName="0. Common - Time Settings")] public string SkipTimeEnd {get;set;}
        [NinjaScriptProperty][Browsable(false)][Display(Name="Flatten Position at Skip Start",Order=6,GroupName="0. Common - Time Settings")] public bool FlattenAtSkipStart {get;set;}
        [NinjaScriptProperty][Display(Name="Use News Skip",Order=7,GroupName="0. Common - Time Settings")] public bool UseNewsSkip {get;set;}
        [Browsable(false)][Display(Name="News Time",Order=8,GroupName="0. Common - Time Settings")] public string NewsTime {get;set;}
        [NinjaScriptProperty][Range(0,240)][Display(Name="News Block Minutes",Order=9,GroupName="0. Common - Time Settings")] public int NewsBlockMinutes {get;set;}
        [NinjaScriptProperty][Browsable(false)][Display(Name="Flatten Position at News Start",Order=10,GroupName="0. Common - Time Settings")] public bool FlattenAtNewsStart {get;set;}
        // ── Webhooks / ProjectX (TopstepX) ─────────────────────────────────
        [NinjaScriptProperty][Display(Name="TradersPost Webhook URL",Order=1,GroupName="0. Common - Webhooks")] public string WebhookUrl {get;set;}
        [NinjaScriptProperty][Display(Name="Webhook Ticker Override",Order=2,GroupName="0. Common - Webhooks")] public string WebhookTickerOverride {get;set;}
        [NinjaScriptProperty][Display(Name="Webhook Provider",Description="Select webhook target: TradersPost or ProjectX.",Order=3,GroupName="0. Common - Webhooks")] public WebhookProvider WebhookProviderType {get;set;}
        [NinjaScriptProperty][Browsable(false)][Display(Name="ProjectX API Base URL",Description="ProjectX gateway base URL.",Order=4,GroupName="0. Common - Webhooks")] public string ProjectXApiBaseUrl {get;set;}
        [Browsable(false)] public bool ProjectXTradeAllAccounts {get;set;}
        [NinjaScriptProperty][Display(Name="ProjectX Username",Description="ProjectX login username.",Order=5,GroupName="0. Common - Webhooks")] public string ProjectXUsername {get;set;}
        [NinjaScriptProperty][Display(Name="ProjectX API Key",Description="ProjectX API key.",Order=6,GroupName="0. Common - Webhooks")] public string ProjectXApiKey {get;set;}
        [NinjaScriptProperty][Display(Name="ProjectX Accounts",Description="Comma-separated ProjectX account ids or exact account names.",Order=7,GroupName="0. Common - Webhooks")] public string ProjectXAccountId {get;set;}
        [NinjaScriptProperty][Browsable(false)][Display(Name="ProjectX Contract ID",Description="Hidden optional override for support/debug use only.",Order=8,GroupName="0. Common - Webhooks")] public string ProjectXContractId {get;set;}
        // ── Global Risk ────────────────────────────────────────────────────
        [Browsable(false)] public bool EnableGlobalMaxDailyLoss {get;set;}
        [NinjaScriptProperty][Browsable(false)][Range(0,double.MaxValue)][Display(Name="Global Max Daily Loss (Points)",Order=2,GroupName="0. Common - Global Risk")] public double GlobalMaxDailyLoss {get;set;}
        [NinjaScriptProperty][Browsable(false)][Display(Name="Enable Global Max Daily Profit",Order=3,GroupName="0. Common - Global Risk")] public bool EnableGlobalMaxDailyProfit {get;set;}
        [NinjaScriptProperty][Browsable(false)][Range(0,double.MaxValue)][Display(Name="Global Max Daily Profit (Points)",Order=4,GroupName="0. Common - Global Risk")] public double GlobalMaxDailyProfit {get;set;}
    ...
    #region Enums
    public enum HugoMAMode_EMA3790           { SMA, EMA, Both }
    public enum HugoEntryMode_EMA3790  { Market, LimitOffset, LimitRetracement }
    public enum HugoTPMode_EMA3790     { FixedTicks, SwingPoint, CandleMultiple }
    public enum HugoBEMode2_EMA3790          { FixedTicks, CandlePercent }
    #endregion
}
