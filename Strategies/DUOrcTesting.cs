...
{
    public class DUOrcTesting : Strategy
    {
        private const string HeartbeatStrategyName = "DUOrcTesting";
        ...
        private const string LongFlipEntrySignal = "DUOrcTestingFlipLong";
        private const string ShortFlipEntrySignal = "DUOrcTestingFlipShort";
        private const string LongSecondaryEntrySignal = "DUOrcTestingLongSecondary";
        private const string ShortSecondaryEntrySignal = "DUOrcTestingShortSecondary";
...
2026-12-30,14:00";
        ...
        protected override void OnStateChange()
        {
            ...
            {
                Name = GetVersionedStrategyName("DUOrcTesting");
        ...
        private bool ShouldSendExitWebhook(Execution execution, string orderName, MarketPosition marketPosition)
        {
            ...
            if (normalized.Equals("Stop loss", StringComparison.OrdinalIgnoreCase)
                ...
                || normalized.StartsWith("DUOrcTesting", StringComparison.OrdinalIgnoreCase))
            {
                return true;
            }
        ...
        private void StartTradeLines(double entryPrice, double stopPrice, double takeProfitPrice, bool hasTakeProfit)
        {
            ...
            tradeLineTagPrefix = string.Format("DUOrcTesting_TradeLine_{0}_{1}_", ++tradeLineTagCounter, CurrentBar);
        ...
        private void UpdateAdxReferenceLines(DM adx, double minThreshold, double maxThreshold)
        {
            ...
            Draw.HorizontalLine(
                adx,
                "DUOrcTesting_ADX_Min_" + adxTagSuffix,
            ...
            Draw.HorizontalLine(
                adx,
                "DUOrcTesting_ADX_Max_" + adxTagSuffix,
        ...
        {
            return !string.IsNullOrEmpty(orderName)
                && orderName.StartsWith("DUOrcTesting", StringComparison.OrdinalIgnoreCase)
        ...
        private void DrawSessionBackgrounds()
        {
            ...
            foreach (SessionSlot slot in ConfigurableSessionSlots)
                DrawSessionBackground(slot, "DUOrcTesting_" + FormatSessionLabel(slot), GetSessionFillBrush(slot));
        }
        ...
        private void DrawNewsWindows(DateTime barTime)
        {
            ...
            for (int i = 0; i < NewsDates.Count; i++)
            {
                ...
                string tagBase = string.Format("DUOrcTesting_News_{0:yyyyMMdd_HHmm}", newsTime);
        ...
        private List<(string label, string value, Brush labelBrush, Brush valueBrush)> BuildInfoLines()
        {
            ...
            lines.Add((string.Format("DUOrcTesting v{0}", GetAddOnVersion()), string.Empty, InfoHeaderTextBrush, Brushes.Transparent));
        ...
        [Display(Name = "Contracts", Description = "Base contracts for Asia 1 entries.", GroupName = "Asia 1", Order = 3)]
        ...
        [NinjaScriptProperty]
        [Display(Name = "Entry Type", Description = "Market enters immediately on a valid signal. EmaLimit places and updates a limit order at EMA21 while normal entry conditions remain valid.", GroupName = "Asia 1", Order = 21)]
        public InitialEntryType AsiaEntryType { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "Entry Close Beyond EMA Points", Description = "0 disables. Long entries require the close at least this many points above EMA; short entries require the close at least this many points below EMA.", GroupName = "Asia 1", Order = 23)]
        public double AsiaEntryCloseBeyondEmaPoints { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Color Blind Entry", Description = "If enabled, long entries only require close above EMA and short entries only require close below EMA. If disabled, entries also require candle color and body crossing EMA.", GroupName = "Asia 1", Order = 24)]
        public bool AsiaColorBlindEntry { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Stop-Out Flip", Description = "When enabled, a stop-loss fill can reverse into the opposite direction when the opposite setup still passes normal entry gates.", GroupName = "Asia 1", Order = 25)]
        public bool AsiaEnableStopOutFlip { get; set; }


        [NinjaScriptProperty]
        [Display(Name = "Entry System", Description = "Primary uses the normal DUOrc entry. Secondary opens an EMA-based working limit order after the normal signal. Both enables both systems.", GroupName = "Asia 1", Order = 26)]
        public EntrySystemMode AsiaEntrySystemMode { get; set; }

        [NinjaScriptProperty]
        [Range(0, int.MaxValue)]
        [Display(Name = "Secondary Contracts", Description = "Contracts used for secondary entries in Asia 1. 0 disables secondary entries for this session.", GroupName = "Asia 1", Order = 27)]
        public int AsiaSecondaryContracts { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "Secondary Entry From EMA", Description = "Distance in points from EMA for the secondary limit entry. Long entries use EMA plus this value; short entries use EMA minus this value.", GroupName = "Asia 1", Order = 28)]
        public double AsiaSecondaryEntryLimitDistanceFromEmaPoints { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "Secondary SL From EMA", Description = "Distance in points from EMA for the secondary stop loss. Long stops use EMA minus this value; short stops use EMA plus this value.", GroupName = "Asia 1", Order = 29)]
        public double AsiaSecondaryEntryStopDistanceFromEmaPoints { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "Secondary TP From EMA", Description = "Distance in points from EMA for the secondary profit target. Long targets use EMA plus this value; short targets use EMA minus this value.", GroupName = "Asia 1", Order = 30)]
        public double AsiaSecondaryEntryTakeProfitDistanceFromEmaPoints { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "Secondary TP Variance", Description = "Maximum random extra profit-target distance in points. Variance applies only to TP, not the secondary entry or stop.", GroupName = "Asia 1", Order = 31)]
        ...
        [NinjaScriptProperty]
        [Range(0, int.MaxValue)]
        [Display(Name = "Contracts", Description = "Base contracts for Asia 2 entries.", GroupName = "Asia 2", Order = 3)]
        ...
        [NinjaScriptProperty]
        [Display(Name = "Entry Type", Description = "Market enters immediately on a valid signal. EmaLimit places and updates a limit order at EMA21 while normal entry conditions remain valid.", GroupName = "Asia 2", Order = 21)]
        public InitialEntryType Asia2EntryType { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "Entry Close Beyond EMA Points", Description = "0 disables. Long entries require the close at least this many points above EMA; short entries require the close at least this many points below EMA.", GroupName = "Asia 2", Order = 23)]
        public double Asia2EntryCloseBeyondEmaPoints { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Color Blind Entry", Description = "If enabled, long entries only require close above EMA and short entries only require close below EMA. If disabled, entries also require candle color and body crossing EMA.", GroupName = "Asia 2", Order = 24)]
        public bool Asia2ColorBlindEntry { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Stop-Out Flip", Description = "When enabled, a stop-loss fill can reverse into the opposite direction when the opposite setup still passes normal entry gates.", GroupName = "Asia 2", Order = 25)]
        public bool Asia2EnableStopOutFlip { get; set; }


        [NinjaScriptProperty]
        [Display(Name = "Entry System", Description = "Primary uses the normal DUOrc entry. Secondary opens an EMA-based working limit order after the normal signal. Both enables both systems.", GroupName = "Asia 2", Order = 26)]
        public EntrySystemMode Asia2EntrySystemMode { get; set; }

        [NinjaScriptProperty]
        [Range(0, int.MaxValue)]
        [Display(Name = "Secondary Contracts", Description = "Contracts used for secondary entries in Asia 2. 0 disables secondary entries for this session.", GroupName = "Asia 2", Order = 27)]
        public int Asia2SecondaryContracts { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "Secondary Entry From EMA", Description = "Distance in points from EMA for the secondary limit entry. Long entries use EMA plus this value; short entries use EMA minus this value.", GroupName = "Asia 2", Order = 28)]
        public double Asia2SecondaryEntryLimitDistanceFromEmaPoints { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "Secondary SL From EMA", Description = "Distance in points from EMA for the secondary stop loss. Long stops use EMA minus this value; short stops use EMA plus this value.", GroupName = "Asia 2", Order = 29)]
        public double Asia2SecondaryEntryStopDistanceFromEmaPoints { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "Secondary TP From EMA", Description = "Distance in points from EMA for the secondary profit target. Long targets use EMA plus this value; short targets use EMA minus this value.", GroupName = "Asia 2", Order = 30)]
        public double Asia2SecondaryEntryTakeProfitDistanceFromEmaPoints { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "Secondary TP Variance", Description = "Maximum random extra profit-target distance in points. Variance applies only to TP, not the secondary entry or stop.", GroupName = "Asia 2", Order = 31)]
        ...
        [NinjaScriptProperty]
        [Range(0, int.MaxValue)]
        [Display(Name = "Contracts", Description = "Base contracts for Asia 3 entries.", GroupName = "Asia 3", Order = 3)]
        ...
        [NinjaScriptProperty]
        [Display(Name = "Entry Type", Description = "Market enters immediately on a valid signal. EmaLimit places and updates a limit order at EMA21 while normal entry conditions remain valid.", GroupName = "Asia 3", Order = 21)]
        public InitialEntryType Asia3EntryType { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "Entry Close Beyond EMA Points", Description = "0 disables. Long entries require the close at least this many points above EMA; short entries require the close at least this many points below EMA.", GroupName = "Asia 3", Order = 23)]
        public double Asia3EntryCloseBeyondEmaPoints { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Color Blind Entry", Description = "If enabled, long entries only require close above EMA and short entries only require close below EMA. If disabled, entries also require candle color and body crossing EMA.", GroupName = "Asia 3", Order = 24)]
        public bool Asia3ColorBlindEntry { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Stop-Out Flip", Description = "When enabled, a stop-loss fill can reverse into the opposite direction when the opposite setup still passes normal entry gates.", GroupName = "Asia 3", Order = 25)]
        public bool Asia3EnableStopOutFlip { get; set; }


        [NinjaScriptProperty]
        [Display(Name = "Entry System", Description = "Primary uses the normal DUOrc entry. Secondary opens an EMA-based working limit order after the normal signal. Both enables both systems.", GroupName = "Asia 3", Order = 26)]
        public EntrySystemMode Asia3EntrySystemMode { get; set; }

        [NinjaScriptProperty]
        [Range(0, int.MaxValue)]
        [Display(Name = "Secondary Contracts", Description = "Contracts used for secondary entries in Asia 3. 0 disables secondary entries for this session.", GroupName = "Asia 3", Order = 27)]
        public int Asia3SecondaryContracts { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "Secondary Entry From EMA", Description = "Distance in points from EMA for the secondary limit entry. Long entries use EMA plus this value; short entries use EMA minus this value.", GroupName = "Asia 3", Order = 28)]
        public double Asia3SecondaryEntryLimitDistanceFromEmaPoints { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "Secondary SL From EMA", Description = "Distance in points from EMA for the secondary stop loss. Long stops use EMA minus this value; short stops use EMA plus this value.", GroupName = "Asia 3", Order = 29)]
        public double Asia3SecondaryEntryStopDistanceFromEmaPoints { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "Secondary TP From EMA", Description = "Distance in points from EMA for the secondary profit target. Long targets use EMA plus this value; short targets use EMA minus this value.", GroupName = "Asia 3", Order = 30)]
        public double Asia3SecondaryEntryTakeProfitDistanceFromEmaPoints { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "Secondary TP Variance", Description = "Maximum random extra profit-target distance in points. Variance applies only to TP, not the secondary entry or stop.", GroupName = "Asia 3", Order = 31)]
        ...
        [NinjaScriptProperty]
        [Display(Name = "Session End", Description = "Europe 1 session end time in chart time zone.", GroupName = "Europe 1", Order = 2)]
        public TimeSpan LondonSessionEnd { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Auto Shift", Description = "Apply Europe 1 DST auto-shift for this session window.", GroupName = "Europe 1", Order = 3)]
        public bool AutoShiftLondon { get; set; }

        [NinjaScriptProperty]
        [Range(1, int.MaxValue)]
        [Display(Name = "EMA Period", Description = "EMA period used by Europe 1 entry and exit logic.", GroupName = "Europe 1", Order = 4)]
        public int LondonEmaPeriod { get; set; }

        [NinjaScriptProperty]
        [Range(0, int.MaxValue)]
        [Display(Name = "Contracts", Description = "Base contracts for Europe 1 entries.", GroupName = "Europe 1", Order = 3)]
        public int LondonContracts { get; set; }

        [NinjaScriptProperty]
        [Range(1, 200)]
        [Display(Name = "ADX Period", Description = "ADX lookback period for the Europe 1 trend filter.", GroupName = "Europe 1", Order = 10)]
        public int LondonAdxPeriod { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, 100.0)]
        [Display(Name = "ADX Min Threshold", Description = "0 disables. Europe 1 entries are allowed only when ADX is greater than or equal to this value.", GroupName = "Europe 1", Order = 11)]
        public double LondonAdxThreshold { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "ADX Max Threshold", Description = "0 disables. Europe 1 entries are allowed only when ADX is less than or equal to this value.", GroupName = "Europe 1", Order = 12)]
        public double LondonAdxMaxThreshold { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "ADX Peak Drawdown Exit", Description = "0 disables. While in a trade, track the highest ADX value and flatten when ADX drops by this many units from that peak.", GroupName = "Europe 1", Order = 14)]
        public double LondonAdxPeakDrawdownExitUnits { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, 100.0)]
        [Display(Name = "ADX Absolute Exit Level", Description = "0 disables. While in a trade, exit immediately when ADX reaches or exceeds this value.", GroupName = "Europe 1", Order = 15)]
        public double LondonAdxAbsoluteExitLevel { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "SL Padding Points", Description = "Stop distance in points from EMA on the opposite side.", GroupName = "Europe 1", Order = 16)]
        public double LondonStopPaddingPoints { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Trail Hard SL", Description = "If enabled, move the hard stop each bar close using EMA plus SL Padding Points. The stop only tightens.", GroupName = "Europe 1", Order = 16)]
        ...
        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "Entry Min Body Points", Description = "0 disables. Initial entry signal candle must have at least this body size in points.", GroupName = "Europe 1", Order = 22)]
        public double LondonEntryMinBodyPoints { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Entry Type", Description = "Market enters immediately on a valid signal. EmaLimit places and updates a limit order at EMA21 while normal entry conditions remain valid.", GroupName = "Europe 1", Order = 21)]
        public InitialEntryType LondonEntryType { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "Entry Close Beyond EMA Points", Description = "0 disables. Long entries require the close at least this many points above EMA; short entries require the close at least this many points below EMA.", GroupName = "Europe 1", Order = 23)]
        public double LondonEntryCloseBeyondEmaPoints { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Color Blind Entry", Description = "If enabled, long entries only require close above EMA and short entries only require close below EMA. If disabled, entries also require candle color and body crossing EMA.", GroupName = "Europe 1", Order = 24)]
        public bool LondonColorBlindEntry { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Stop-Out Flip", Description = "When enabled, a stop-loss fill can reverse into the opposite direction when the opposite setup still passes normal entry gates.", GroupName = "Europe 1", Order = 25)]
        public bool LondonEnableStopOutFlip { get; set; }


        [NinjaScriptProperty]
        [Display(Name = "Entry System", Description = "Primary uses the normal DUOrc entry. Secondary opens an EMA-based working limit order after the normal signal. Both enables both systems.", GroupName = "Europe 1", Order = 26)]
        public EntrySystemMode LondonEntrySystemMode { get; set; }

        [NinjaScriptProperty]
        [Range(0, int.MaxValue)]
        [Display(Name = "Secondary Contracts", Description = "Contracts used for secondary entries in Europe 1. 0 disables secondary entries for this session.", GroupName = "Europe 1", Order = 27)]
        public int LondonSecondaryContracts { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "Secondary Entry From EMA", Description = "Distance in points from EMA for the secondary limit entry. Long entries use EMA plus this value; short entries use EMA minus this value.", GroupName = "Europe 1", Order = 28)]
        public double LondonSecondaryEntryLimitDistanceFromEmaPoints { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "Secondary SL From EMA", Description = "Distance in points from EMA for the secondary stop loss. Long stops use EMA minus this value; short stops use EMA plus this value.", GroupName = "Europe 1", Order = 29)]
        public double LondonSecondaryEntryStopDistanceFromEmaPoints { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "Secondary TP From EMA", Description = "Distance in points from EMA for the secondary profit target. Long targets use EMA plus this value; short targets use EMA minus this value.", GroupName = "Europe 1", Order = 30)]
        public double LondonSecondaryEntryTakeProfitDistanceFromEmaPoints { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "Secondary TP Variance", Description = "Maximum random extra profit-target distance in points. Variance applies only to TP, not the secondary entry or stop.", GroupName = "Europe 1", Order = 31)]
        public double LondonSecondaryTakeProfitVariancePoints { get; set; }
        [NinjaScriptProperty]
        [Range(0, int.MaxValue)]
        [Display(Name = "Candle Reversal Exit Bars", Description = "0 disables. After this many bars held, short exits on bullish close above the most recent bearish candle high; long exits on bearish close below the most recent bullish candle low.", GroupName = "Europe 1", Order = 36)]
        public int LondonCandleReversalExitBars { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "Candle Reversal Close Beyond Points", Description = "0 uses the candle high/low exactly. Long exits require a close this many points below the reference bullish candle low; short exits require this many points above the reference bearish candle high.", GroupName = "Europe 1", Order = 37)]
        public double LondonCandleReversalCloseBeyondPoints { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "Candle Reversal Min Body Points", Description = "0 disables. Reference bullish/bearish candles must have at least this body size in points to count for the candle reversal exit.", GroupName = "Europe 1", Order = 38)]
        public double LondonCandleReversalMinBodyPoints { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "ATR Min Threshold", Description = "0 disables. Block new Europe 1 entries while ATR(14) is below this value.", GroupName = "Europe 1", Order = 40)]
        ...
        [NinjaScriptProperty]
        [Display(Name = "Session End", Description = "America 1 session end time in chart time zone.", GroupName = "America 1", Order = 2)]
        public TimeSpan NewYorkSessionEnd { get; set; }

        [NinjaScriptProperty]
        [Range(1, int.MaxValue)]
        [Display(Name = "EMA Period", Description = "EMA period used by America 1 entry and exit logic.", GroupName = "America 1", Order = 5)]
        public int NewYorkEmaPeriod { get; set; }

        [NinjaScriptProperty]
        [Range(0, int.MaxValue)]
        [Display(Name = "Contracts", Description = "Base contracts for America 1 entries.", GroupName = "America 1", Order = 3)]
        public int NewYorkContracts { get; set; }

        [NinjaScriptProperty]
        [Range(1, 200)]
        [Display(Name = "ADX Period", Description = "ADX lookback period for the America 1 trend filter.", GroupName = "America 1", Order = 11)]
        public int NewYorkAdxPeriod { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, 100.0)]
        [Display(Name = "ADX Min Threshold", Description = "0 disables. America 1 entries are allowed only when ADX is greater than or equal to this value.", GroupName = "America 1", Order = 12)]
        public double NewYorkAdxThreshold { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "ADX Max Threshold", Description = "0 disables. America 1 entries are allowed only when ADX is less than or equal to this value.", GroupName = "America 1", Order = 13)]
        public double NewYorkAdxMaxThreshold { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "ADX Peak Drawdown Exit", Description = "0 disables. While in a trade, track the highest ADX value and flatten when ADX drops by this many units from that peak.", GroupName = "America 1", Order = 15)]
        public double NewYorkAdxPeakDrawdownExitUnits { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, 100.0)]
        [Display(Name = "ADX Absolute Exit Level", Description = "0 disables. While in a trade, exit immediately when ADX reaches or exceeds this value.", GroupName = "America 1", Order = 16)]
        public double NewYorkAdxAbsoluteExitLevel { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "SL Padding Points", Description = "Stop distance in points from EMA on the opposite side.", GroupName = "America 1", Order = 17)]
        public double NewYorkStopPaddingPoints { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Trail Hard SL", Description = "If enabled, move the hard stop each bar close using EMA plus SL Padding Points. The stop only tightens.", GroupName = "America 1", Order = 17)]
        ...
        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "Entry Min Body Points", Description = "0 disables. Initial entry signal candle must have at least this body size in points.", GroupName = "America 1", Order = 22)]
        public double NewYorkEntryMinBodyPoints { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Entry Type", Description = "Market enters immediately on a valid signal. EmaLimit places and updates a limit order at EMA21 while normal entry conditions remain valid.", GroupName = "America 1", Order = 21)]
        public InitialEntryType NewYorkEntryType { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "Entry Close Beyond EMA Points", Description = "0 disables. Long entries require the close at least this many points above EMA; short entries require the close at least this many points below EMA.", GroupName = "America 1", Order = 23)]
        public double NewYorkEntryCloseBeyondEmaPoints { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Color Blind Entry", Description = "If enabled, long entries only require close above EMA and short entries only require close below EMA. If disabled, entries also require candle color and body crossing EMA.", GroupName = "America 1", Order = 24)]
        public bool NewYorkColorBlindEntry { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Stop-Out Flip", Description = "When enabled, a stop-loss fill can reverse into the opposite direction when the opposite setup still passes normal entry gates.", GroupName = "America 1", Order = 25)]
        public bool NewYorkEnableStopOutFlip { get; set; }


        [NinjaScriptProperty]
        [Display(Name = "Entry System", Description = "Primary uses the normal DUOrc entry. Secondary opens an EMA-based working limit order after the normal signal. Both enables both systems.", GroupName = "America 1", Order = 26)]
        public EntrySystemMode NewYorkEntrySystemMode { get; set; }

        [NinjaScriptProperty]
        [Range(0, int.MaxValue)]
        [Display(Name = "Secondary Contracts", Description = "Contracts used for secondary entries in America 1. 0 disables secondary entries for this session.", GroupName = "America 1", Order = 27)]
        public int NewYorkSecondaryContracts { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "Secondary Entry From EMA", Description = "Distance in points from EMA for the secondary limit entry. Long entries use EMA plus this value; short entries use EMA minus this value.", GroupName = "America 1", Order = 28)]
        public double NewYorkSecondaryEntryLimitDistanceFromEmaPoints { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "Secondary SL From EMA", Description = "Distance in points from EMA for the secondary stop loss. Long stops use EMA minus this value; short stops use EMA plus this value.", GroupName = "America 1", Order = 29)]
        public double NewYorkSecondaryEntryStopDistanceFromEmaPoints { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "Secondary TP From EMA", Description = "Distance in points from EMA for the secondary profit target. Long targets use EMA plus this value; short targets use EMA minus this value.", GroupName = "America 1", Order = 30)]
        public double NewYorkSecondaryEntryTakeProfitDistanceFromEmaPoints { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "Secondary TP Variance", Description = "Maximum random extra profit-target distance in points. Variance applies only to TP, not the secondary entry or stop.", GroupName = "America 1", Order = 31)]
        public double NewYorkSecondaryTakeProfitVariancePoints { get; set; }
        [NinjaScriptProperty]
        [Range(0, int.MaxValue)]
        [Display(Name = "Candle Reversal Exit Bars", Description = "0 disables. After this many bars held, short exits on bullish close above the most recent bearish candle high; long exits on bearish close below the most recent bullish candle low.", GroupName = "America 1", Order = 37)]
        public int NewYorkCandleReversalExitBars { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "Candle Reversal Close Beyond Points", Description = "0 uses the candle high/low exactly. Long exits require a close this many points below the reference bullish candle low; short exits require this many points above the reference bearish candle high.", GroupName = "America 1", Order = 38)]
        public double NewYorkCandleReversalCloseBeyondPoints { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "Candle Reversal Min Body Points", Description = "0 disables. Reference bullish/bearish candles must have at least this body size in points to count for the candle reversal exit.", GroupName = "America 1", Order = 39)]
        public double NewYorkCandleReversalMinBodyPoints { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "ATR Min Threshold", Description = "0 disables. Block new America 1 entries while ATR(14) is below this value.", GroupName = "America 1", Order = 41)]
        ...
        [NinjaScriptProperty]
        [Display(Name = "Session End", Description = "America 2 session end time in chart time zone.", GroupName = "America 2", Order = 2)]
        public TimeSpan NewYork2SessionEnd { get; set; }

        [NinjaScriptProperty]
        [Range(1, int.MaxValue)]
        [Display(Name = "EMA Period", Description = "EMA period used by America 2 entry and exit logic.", GroupName = "America 2", Order = 5)]
        public int NewYork2EmaPeriod { get; set; }

        [NinjaScriptProperty]
        [Range(0, int.MaxValue)]
        [Display(Name = "Contracts", Description = "Base contracts for America 2 entries.", GroupName = "America 2", Order = 3)]
        public int NewYork2Contracts { get; set; }

        [NinjaScriptProperty]
        [Range(1, 200)]
        [Display(Name = "ADX Period", Description = "ADX lookback period for the America 2 trend filter.", GroupName = "America 2", Order = 11)]
        public int NewYork2AdxPeriod { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, 100.0)]
        [Display(Name = "ADX Min Threshold", Description = "0 disables. America 2 entries are allowed only when ADX is greater than or equal to this value.", GroupName = "America 2", Order = 12)]
        public double NewYork2AdxThreshold { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "ADX Max Threshold", Description = "0 disables. America 2 entries are allowed only when ADX is less than or equal to this value.", GroupName = "America 2", Order = 13)]
        public double NewYork2AdxMaxThreshold { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "ADX Peak Drawdown Exit", Description = "0 disables. While in a trade, track the highest ADX value and flatten when ADX drops by this many units from that peak.", GroupName = "America 2", Order = 15)]
        public double NewYork2AdxPeakDrawdownExitUnits { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, 100.0)]
        [Display(Name = "ADX Absolute Exit Level", Description = "0 disables. While in a trade, exit immediately when ADX reaches or exceeds this value.", GroupName = "America 2", Order = 16)]
        public double NewYork2AdxAbsoluteExitLevel { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "SL Padding Points", Description = "Stop distance in points from EMA on the opposite side.", GroupName = "America 2", Order = 17)]
        public double NewYork2StopPaddingPoints { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Trail Hard SL", Description = "If enabled, move the hard stop each bar close using EMA plus SL Padding Points. The stop only tightens.", GroupName = "America 2", Order = 17)]
        ...
        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "Entry Min Body Points", Description = "0 disables. Initial entry signal candle must have at least this body size in points.", GroupName = "America 2", Order = 22)]
        public double NewYork2EntryMinBodyPoints { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Entry Type", Description = "Market enters immediately on a valid signal. EmaLimit places and updates a limit order at EMA21 while normal entry conditions remain valid.", GroupName = "America 2", Order = 21)]
        public InitialEntryType NewYork2EntryType { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "Entry Close Beyond EMA Points", Description = "0 disables. Long entries require the close at least this many points above EMA; short entries require the close at least this many points below EMA.", GroupName = "America 2", Order = 23)]
        public double NewYork2EntryCloseBeyondEmaPoints { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Color Blind Entry", Description = "If enabled, long entries only require close above EMA and short entries only require close below EMA. If disabled, entries also require candle color and body crossing EMA.", GroupName = "America 2", Order = 24)]
        public bool NewYork2ColorBlindEntry { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Stop-Out Flip", Description = "When enabled, a stop-loss fill can reverse into the opposite direction when the opposite setup still passes normal entry gates.", GroupName = "America 2", Order = 25)]
        public bool NewYork2EnableStopOutFlip { get; set; }


        [NinjaScriptProperty]
        [Display(Name = "Entry System", Description = "Primary uses the normal DUOrc entry. Secondary opens an EMA-based working limit order after the normal signal. Both enables both systems.", GroupName = "America 2", Order = 26)]
        public EntrySystemMode NewYork2EntrySystemMode { get; set; }

        [NinjaScriptProperty]
        [Range(0, int.MaxValue)]
        [Display(Name = "Secondary Contracts", Description = "Contracts used for secondary entries in America 2. 0 disables secondary entries for this session.", GroupName = "America 2", Order = 27)]
        public int NewYork2SecondaryContracts { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "Secondary Entry From EMA", Description = "Distance in points from EMA for the secondary limit entry. Long entries use EMA plus this value; short entries use EMA minus this value.", GroupName = "America 2", Order = 28)]
        public double NewYork2SecondaryEntryLimitDistanceFromEmaPoints { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "Secondary SL From EMA", Description = "Distance in points from EMA for the secondary stop loss. Long stops use EMA minus this value; short stops use EMA plus this value.", GroupName = "America 2", Order = 29)]
        public double NewYork2SecondaryEntryStopDistanceFromEmaPoints { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "Secondary TP From EMA", Description = "Distance in points from EMA for the secondary profit target. Long targets use EMA plus this value; short targets use EMA minus this value.", GroupName = "America 2", Order = 30)]
        public double NewYork2SecondaryEntryTakeProfitDistanceFromEmaPoints { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "Secondary TP Variance", Description = "Maximum random extra profit-target distance in points. Variance applies only to TP, not the secondary entry or stop.", GroupName = "America 2", Order = 31)]
        public double NewYork2SecondaryTakeProfitVariancePoints { get; set; }
        [NinjaScriptProperty]
        [Range(0, int.MaxValue)]
        [Display(Name = "Candle Reversal Exit Bars", Description = "0 disables. After this many bars held, short exits on bullish close above the most recent bearish candle high; long exits on bearish close below the most recent bullish candle low.", GroupName = "America 2", Order = 37)]
        public int NewYork2CandleReversalExitBars { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "Candle Reversal Close Beyond Points", Description = "0 uses the candle high/low exactly. Long exits require a close this many points below the reference bullish candle low; short exits require this many points above the reference bearish candle high.", GroupName = "America 2", Order = 38)]
        public double NewYork2CandleReversalCloseBeyondPoints { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "Candle Reversal Min Body Points", Description = "0 disables. Reference bullish/bearish candles must have at least this body size in points to count for the candle reversal exit.", GroupName = "America 2", Order = 39)]
        public double NewYork2CandleReversalMinBodyPoints { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "ATR Min Threshold", Description = "0 disables. Block new America 2 entries while ATR(14) is below this value.", GroupName = "America 2", Order = 41)]
        ...
        [NinjaScriptProperty]
        [Display(Name = "Session End", Description = "America 3 session end time in chart time zone.", GroupName = "America 3", Order = 2)]
        public TimeSpan NewYork3SessionEnd { get; set; }

        [NinjaScriptProperty]
        [Range(1, int.MaxValue)]
        [Display(Name = "EMA Period", Description = "EMA period used by America 3 entry and exit logic.", GroupName = "America 3", Order = 5)]
        public int NewYork3EmaPeriod { get; set; }

        [NinjaScriptProperty]
        [Range(0, int.MaxValue)]
        [Display(Name = "Contracts", Description = "Base contracts for America 3 entries.", GroupName = "America 3", Order = 3)]
        public int NewYork3Contracts { get; set; }

        [NinjaScriptProperty]
        [Range(1, 200)]
        [Display(Name = "ADX Period", Description = "ADX lookback period for the America 3 trend filter.", GroupName = "America 3", Order = 11)]
        public int NewYork3AdxPeriod { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, 100.0)]
        [Display(Name = "ADX Min Threshold", Description = "0 disables. America 3 entries are allowed only when ADX is greater than or equal to this value.", GroupName = "America 3", Order = 12)]
        public double NewYork3AdxThreshold { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "ADX Max Threshold", Description = "0 disables. America 3 entries are allowed only when ADX is less than or equal to this value.", GroupName = "America 3", Order = 13)]
        public double NewYork3AdxMaxThreshold { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "ADX Peak Drawdown Exit", Description = "0 disables. While in a trade, track the highest ADX value and flatten when ADX drops by this many units from that peak.", GroupName = "America 3", Order = 15)]
        public double NewYork3AdxPeakDrawdownExitUnits { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, 100.0)]
        [Display(Name = "ADX Absolute Exit Level", Description = "0 disables. While in a trade, exit immediately when ADX reaches or exceeds this value.", GroupName = "America 3", Order = 16)]
        public double NewYork3AdxAbsoluteExitLevel { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "SL Padding Points", Description = "Stop distance in points from EMA on the opposite side.", GroupName = "America 3", Order = 17)]
        public double NewYork3StopPaddingPoints { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Trail Hard SL", Description = "If enabled, move the hard stop each bar close using EMA plus SL Padding Points. The stop only tightens.", GroupName = "America 3", Order = 17)]
        ...
        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "Entry Min Body Points", Description = "0 disables. Initial entry signal candle must have at least this body size in points.", GroupName = "America 3", Order = 22)]
        public double NewYork3EntryMinBodyPoints { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Entry Type", Description = "Market enters immediately on a valid signal. EmaLimit places and updates a limit order at EMA21 while normal entry conditions remain valid.", GroupName = "America 3", Order = 21)]
        public InitialEntryType NewYork3EntryType { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "Entry Close Beyond EMA Points", Description = "0 disables. Long entries require the close at least this many points above EMA; short entries require the close at least this many points below EMA.", GroupName = "America 3", Order = 23)]
        public double NewYork3EntryCloseBeyondEmaPoints { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Color Blind Entry", Description = "If enabled, long entries only require close above EMA and short entries only require close below EMA. If disabled, entries also require candle color and body crossing EMA.", GroupName = "America 3", Order = 24)]
        public bool NewYork3ColorBlindEntry { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Stop-Out Flip", Description = "When enabled, a stop-loss fill can reverse into the opposite direction when the opposite setup still passes normal entry gates.", GroupName = "America 3", Order = 25)]
        public bool NewYork3EnableStopOutFlip { get; set; }


        [NinjaScriptProperty]
        [Display(Name = "Entry System", Description = "Primary uses the normal DUOrc entry. Secondary opens an EMA-based working limit order after the normal signal. Both enables both systems.", GroupName = "America 3", Order = 26)]
        public EntrySystemMode NewYork3EntrySystemMode { get; set; }

        [NinjaScriptProperty]
        [Range(0, int.MaxValue)]
        [Display(Name = "Secondary Contracts", Description = "Contracts used for secondary entries in America 3. 0 disables secondary entries for this session.", GroupName = "America 3", Order = 27)]
        public int NewYork3SecondaryContracts { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "Secondary Entry From EMA", Description = "Distance in points from EMA for the secondary limit entry. Long entries use EMA plus this value; short entries use EMA minus this value.", GroupName = "America 3", Order = 28)]
        public double NewYork3SecondaryEntryLimitDistanceFromEmaPoints { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "Secondary SL From EMA", Description = "Distance in points from EMA for the secondary stop loss. Long stops use EMA minus this value; short stops use EMA plus this value.", GroupName = "America 3", Order = 29)]
        public double NewYork3SecondaryEntryStopDistanceFromEmaPoints { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "Secondary TP From EMA", Description = "Distance in points from EMA for the secondary profit target. Long targets use EMA plus this value; short targets use EMA minus this value.", GroupName = "America 3", Order = 30)]
        public double NewYork3SecondaryEntryTakeProfitDistanceFromEmaPoints { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "Secondary TP Variance", Description = "Maximum random extra profit-target distance in points. Variance applies only to TP, not the secondary entry or stop.", GroupName = "America 3", Order = 31)]
        public double NewYork3SecondaryTakeProfitVariancePoints { get; set; }
        [NinjaScriptProperty]
        [Range(0, int.MaxValue)]
        [Display(Name = "Candle Reversal Exit Bars", Description = "0 disables. After this many bars held, short exits on bullish close above the most recent bearish candle high; long exits on bearish close below the most recent bullish candle low.", GroupName = "America 3", Order = 37)]
        public int NewYork3CandleReversalExitBars { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "Candle Reversal Close Beyond Points", Description = "0 uses the candle high/low exactly. Long exits require a close this many points below the reference bullish candle low; short exits require this many points above the reference bearish candle high.", GroupName = "America 3", Order = 38)]
        public double NewYork3CandleReversalCloseBeyondPoints { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "Candle Reversal Min Body Points", Description = "0 disables. Reference bullish/bearish candles must have at least this body size in points to count for the candle reversal exit.", GroupName = "America 3", Order = 39)]
        public double NewYork3CandleReversalMinBodyPoints { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "ATR Min Threshold", Description = "0 disables. Block new America 3 entries while ATR(14) is below this value.", GroupName = "America 3", Order = 41)]
        public double NewYork3AtrMinimum { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Session Start", Description = "America 4 session start time in chart time zone.", GroupName = "America 4", Order = 1)]
        public TimeSpan NewYork4SessionStart { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Session End", Description = "America 4 session end time in chart time zone.", GroupName = "America 4", Order = 2)]
        public TimeSpan NewYork4SessionEnd { get; set; }

        [NinjaScriptProperty]
        [Range(1, int.MaxValue)]
        [Display(Name = "EMA Period", Description = "EMA period used by America 4 entry and exit logic.", GroupName = "America 4", Order = 5)]
        public int NewYork4EmaPeriod { get; set; }

        [NinjaScriptProperty]
        [Range(0, int.MaxValue)]
        [Display(Name = "Contracts", Description = "Base contracts for America 4 entries.", GroupName = "America 4", Order = 3)]
        public int NewYork4Contracts { get; set; }

        [NinjaScriptProperty]
        [Range(1, 200)]
        [Display(Name = "ADX Period", Description = "ADX lookback period for the America 4 trend filter.", GroupName = "America 4", Order = 11)]
        public int NewYork4AdxPeriod { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, 100.0)]
        [Display(Name = "ADX Min Threshold", Description = "0 disables. America 4 entries are allowed only when ADX is greater than or equal to this value.", GroupName = "America 4", Order = 12)]
        public double NewYork4AdxThreshold { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "ADX Max Threshold", Description = "0 disables. America 4 entries are allowed only when ADX is less than or equal to this value.", GroupName = "America 4", Order = 13)]
        public double NewYork4AdxMaxThreshold { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "ADX Peak Drawdown Exit", Description = "0 disables. While in a trade, track the highest ADX value and flatten when ADX drops by this many units from that peak.", GroupName = "America 4", Order = 15)]
        public double NewYork4AdxPeakDrawdownExitUnits { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, 100.0)]
        [Display(Name = "ADX Absolute Exit Level", Description = "0 disables. While in a trade, exit immediately when ADX reaches or exceeds this value.", GroupName = "America 4", Order = 16)]
        public double NewYork4AdxAbsoluteExitLevel { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "SL Padding Points", Description = "Stop distance in points from EMA on the opposite side.", GroupName = "America 4", Order = 17)]
        public double NewYork4StopPaddingPoints { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Trail Hard SL", Description = "If enabled, move the hard stop each bar close using EMA plus SL Padding Points. The stop only tightens.", GroupName = "America 4", Order = 17)]
        public bool NewYork4TrailHardStop { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "Take Profit (Points)", Description = "0 disables. Exit when unrealized profit reaches this many points from average entry price.", GroupName = "America 4", Order = 19)]
        public double NewYork4TakeProfitPoints { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "Entry Min Body Points", Description = "0 disables. Initial entry signal candle must have at least this body size in points.", GroupName = "America 4", Order = 22)]
        public double NewYork4EntryMinBodyPoints { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Entry Type", Description = "Market enters immediately on a valid signal. EmaLimit places and updates a limit order at EMA21 while normal entry conditions remain valid.", GroupName = "America 4", Order = 21)]
        public InitialEntryType NewYork4EntryType { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "Entry Close Beyond EMA Points", Description = "0 disables. Long entries require the close at least this many points above EMA; short entries require the close at least this many points below EMA.", GroupName = "America 4", Order = 23)]
        public double NewYork4EntryCloseBeyondEmaPoints { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Color Blind Entry", Description = "If enabled, long entries only require close above EMA and short entries only require close below EMA. If disabled, entries also require candle color and body crossing EMA.", GroupName = "America 4", Order = 24)]
        public bool NewYork4ColorBlindEntry { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Stop-Out Flip", Description = "When enabled, a stop-loss fill can reverse into the opposite direction when the opposite setup still passes normal entry gates.", GroupName = "America 4", Order = 25)]
        public bool NewYork4EnableStopOutFlip { get; set; }


        [NinjaScriptProperty]
        [Display(Name = "Entry System", Description = "Primary uses the normal DUOrc entry. Secondary opens an EMA-based working limit order after the normal signal. Both enables both systems.", GroupName = "America 4", Order = 26)]
        public EntrySystemMode NewYork4EntrySystemMode { get; set; }

        [NinjaScriptProperty]
        [Range(0, int.MaxValue)]
        [Display(Name = "Secondary Contracts", Description = "Contracts used for secondary entries in America 4. 0 disables secondary entries for this session.", GroupName = "America 4", Order = 27)]
        public int NewYork4SecondaryContracts { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "Secondary Entry From EMA", Description = "Distance in points from EMA for the secondary limit entry. Long entries use EMA plus this value; short entries use EMA minus this value.", GroupName = "America 4", Order = 28)]
        public double NewYork4SecondaryEntryLimitDistanceFromEmaPoints { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "Secondary SL From EMA", Description = "Distance in points from EMA for the secondary stop loss. Long stops use EMA minus this value; short stops use EMA plus this value.", GroupName = "America 4", Order = 29)]
        public double NewYork4SecondaryEntryStopDistanceFromEmaPoints { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "Secondary TP From EMA", Description = "Distance in points from EMA for the secondary profit target. Long targets use EMA plus this value; short targets use EMA minus this value.", GroupName = "America 4", Order = 30)]
        public double NewYork4SecondaryEntryTakeProfitDistanceFromEmaPoints { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "Secondary TP Variance", Description = "Maximum random extra profit-target distance in points. Variance applies only to TP, not the secondary entry or stop.", GroupName = "America 4", Order = 31)]
        public double NewYork4SecondaryTakeProfitVariancePoints { get; set; }
        [NinjaScriptProperty]
        [Range(0, int.MaxValue)]
        [Display(Name = "Candle Reversal Exit Bars", Description = "0 disables. After this many bars held, short exits on bullish close above the most recent bearish candle high; long exits on bearish close below the most recent bullish candle low.", GroupName = "America 4", Order = 37)]
        public int NewYork4CandleReversalExitBars { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "Candle Reversal Close Beyond Points", Description = "0 uses the candle high/low exactly. Long exits require a close this many points below the reference bullish candle low; short exits require this many points above the reference bearish candle high.", GroupName = "America 4", Order = 38)]
        public double NewYork4CandleReversalCloseBeyondPoints { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "Candle Reversal Min Body Points", Description = "0 disables. Reference bullish/bearish candles must have at least this body size in points to count for the candle reversal exit.", GroupName = "America 4", Order = 39)]
        public double NewYork4CandleReversalMinBodyPoints { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "ATR Min Threshold", Description = "0 disables. Block new America 4 entries while ATR(14) is below this value.", GroupName = "America 4", Order = 41)]
        public double NewYork4AtrMinimum { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Session Start", Description = "America 5 session start time in chart time zone.", GroupName = "America 5", Order = 1)]
        public TimeSpan NewYork5SessionStart { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Session End", Description = "America 5 session end time in chart time zone.", GroupName = "America 5", Order = 2)]
        public TimeSpan NewYork5SessionEnd { get; set; }

        [NinjaScriptProperty]
        [Range(1, int.MaxValue)]
        [Display(Name = "EMA Period", Description = "EMA period used by America 5 entry and exit logic.", GroupName = "America 5", Order = 5)]
        public int NewYork5EmaPeriod { get; set; }

        [NinjaScriptProperty]
        [Range(0, int.MaxValue)]
        [Display(Name = "Contracts", Description = "Base contracts for America 5 entries.", GroupName = "America 5", Order = 3)]
        public int NewYork5Contracts { get; set; }

        [NinjaScriptProperty]
        [Range(1, 200)]
        [Display(Name = "ADX Period", Description = "ADX lookback period for the America 5 trend filter.", GroupName = "America 5", Order = 11)]
        public int NewYork5AdxPeriod { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, 100.0)]
        [Display(Name = "ADX Min Threshold", Description = "0 disables. America 5 entries are allowed only when ADX is greater than or equal to this value.", GroupName = "America 5", Order = 12)]
        public double NewYork5AdxThreshold { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "ADX Max Threshold", Description = "0 disables. America 5 entries are allowed only when ADX is less than or equal to this value.", GroupName = "America 5", Order = 13)]
        public double NewYork5AdxMaxThreshold { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "ADX Peak Drawdown Exit", Description = "0 disables. While in a trade, track the highest ADX value and flatten when ADX drops by this many units from that peak.", GroupName = "America 5", Order = 15)]
        public double NewYork5AdxPeakDrawdownExitUnits { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, 100.0)]
        [Display(Name = "ADX Absolute Exit Level", Description = "0 disables. While in a trade, exit immediately when ADX reaches or exceeds this value.", GroupName = "America 5", Order = 16)]
        public double NewYork5AdxAbsoluteExitLevel { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "SL Padding Points", Description = "Stop distance in points from EMA on the opposite side.", GroupName = "America 5", Order = 17)]
        public double NewYork5StopPaddingPoints { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Trail Hard SL", Description = "If enabled, move the hard stop each bar close using EMA plus SL Padding Points. The stop only tightens.", GroupName = "America 5", Order = 17)]
        public bool NewYork5TrailHardStop { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "Take Profit (Points)", Description = "0 disables. Exit when unrealized profit reaches this many points from average entry price.", GroupName = "America 5", Order = 19)]
        public double NewYork5TakeProfitPoints { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "Entry Min Body Points", Description = "0 disables. Initial entry signal candle must have at least this body size in points.", GroupName = "America 5", Order = 22)]
        public double NewYork5EntryMinBodyPoints { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Entry Type", Description = "Market enters immediately on a valid signal. EmaLimit places and updates a limit order at EMA21 while normal entry conditions remain valid.", GroupName = "America 5", Order = 21)]
        public InitialEntryType NewYork5EntryType { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "Entry Close Beyond EMA Points", Description = "0 disables. Long entries require the close at least this many points above EMA; short entries require the close at least this many points below EMA.", GroupName = "America 5", Order = 23)]
        public double NewYork5EntryCloseBeyondEmaPoints { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Color Blind Entry", Description = "If enabled, long entries only require close above EMA and short entries only require close below EMA. If disabled, entries also require candle color and body crossing EMA.", GroupName = "America 5", Order = 24)]
        public bool NewYork5ColorBlindEntry { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Stop-Out Flip", Description = "When enabled, a stop-loss fill can reverse into the opposite direction when the opposite setup still passes normal entry gates.", GroupName = "America 5", Order = 25)]
        public bool NewYork5EnableStopOutFlip { get; set; }


        [NinjaScriptProperty]
        [Display(Name = "Entry System", Description = "Primary uses the normal DUOrc entry. Secondary opens an EMA-based working limit order after the normal signal. Both enables both systems.", GroupName = "America 5", Order = 26)]
        public EntrySystemMode NewYork5EntrySystemMode { get; set; }

        [NinjaScriptProperty]
        [Range(0, int.MaxValue)]
        [Display(Name = "Secondary Contracts", Description = "Contracts used for secondary entries in America 5. 0 disables secondary entries for this session.", GroupName = "America 5", Order = 27)]
        public int NewYork5SecondaryContracts { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "Secondary Entry From EMA", Description = "Distance in points from EMA for the secondary limit entry. Long entries use EMA plus this value; short entries use EMA minus this value.", GroupName = "America 5", Order = 28)]
        public double NewYork5SecondaryEntryLimitDistanceFromEmaPoints { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "Secondary SL From EMA", Description = "Distance in points from EMA for the secondary stop loss. Long stops use EMA minus this value; short stops use EMA plus this value.", GroupName = "America 5", Order = 29)]
        public double NewYork5SecondaryEntryStopDistanceFromEmaPoints { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "Secondary TP From EMA", Description = "Distance in points from EMA for the secondary profit target. Long targets use EMA plus this value; short targets use EMA minus this value.", GroupName = "America 5", Order = 30)]
        public double NewYork5SecondaryEntryTakeProfitDistanceFromEmaPoints { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "Secondary TP Variance", Description = "Maximum random extra profit-target distance in points. Variance applies only to TP, not the secondary entry or stop.", GroupName = "America 5", Order = 31)]
        public double NewYork5SecondaryTakeProfitVariancePoints { get; set; }
        [NinjaScriptProperty]
        [Range(0, int.MaxValue)]
        [Display(Name = "Candle Reversal Exit Bars", Description = "0 disables. After this many bars held, short exits on bullish close above the most recent bearish candle high; long exits on bearish close below the most recent bullish candle low.", GroupName = "America 5", Order = 37)]
        public int NewYork5CandleReversalExitBars { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "Candle Reversal Close Beyond Points", Description = "0 uses the candle high/low exactly. Long exits require a close this many points below the reference bullish candle low; short exits require this many points above the reference bearish candle high.", GroupName = "America 5", Order = 38)]
        public double NewYork5CandleReversalCloseBeyondPoints { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "Candle Reversal Min Body Points", Description = "0 disables. Reference bullish/bearish candles must have at least this body size in points to count for the candle reversal exit.", GroupName = "America 5", Order = 39)]
        public double NewYork5CandleReversalMinBodyPoints { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "ATR Min Threshold", Description = "0 disables. Block new America 5 entries while ATR(14) is below this value.", GroupName = "America 5", Order = 41)]
        ...
        [NinjaScriptProperty]
        [Display(Name = "Secondary Trail Stop", Description = "If enabled, secondary entries ignore normal primary exits and trail their stop every 5-minute close at the original EMA stop distance. Mandatory risk/session exits still flatten.", GroupName = "13. Risk", Order = 5)]
