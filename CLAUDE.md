...
- Most strategies come in pairs: `XXX.cs` (public/closed) and `XXXTesting.cs`
  (full logic). Families: HUGO, MICH, ORBO, and other active paired bots still
  follow this model.
- DUO and DUOrc are exceptions. They are maintained directly in
  `Strategies/DUO.cs` and `Strategies/DUOrc.cs`; do not create or restore
  separate DUO/DUOrc testing variants unless the user explicitly asks.
- `DUO.cs` is the canonical reference implementation for most shared patterns.
