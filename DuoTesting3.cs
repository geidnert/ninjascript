...
{
    public class DuoTesting3 : Strategy
    {
        public DuoTesting3()
        {
            VendorLicense(337);
        ...
        private sealed class AsiaAdxSlopeDropdownConverter : System.ComponentModel.DoubleConverter
        {
            ...
            {
                0.8, 1.33, 1.34, 1.35, 1.36, 1.37, 1.48, 1.49, 1.50, 1.51, 1.52, 1.53, 1.54, 1.55
            };
        ...
        private double v1EntryAdxAtFill;
...
2026-12-31,08:30";
        ...
        protected override void OnStateChange()
        {
            if (State == State.SetDefaults)
            {
                ...
                EntryModel = EntryModelMode.EntryModelV1;
                ArmDynamicExitsAfterAdxRiseV1 = 0.0;
                EntryModelV2_SwingCandlesBefore = 0;
        ...
        protected override void OnBarUpdate()
        {
            ...
            bool canTradeNowNoAdx = inActiveSessionNow && !inNewsSkipNow && !inNySkipNow && !isAsiaSundayBlockedNow && !accountBlocked && maxTradesPass;
            ...
            if (EntryModel == EntryModelMode.EntryModelV2)
            {
                ...
                if (!v2Prepared)
                {
                    ...
                    bool v2PrepareSlopePass = activeEmaMinSlopePointsPerBar <= 0.0
                        || Math.Abs(GetEmaSlopePointsPerBar()) >= activeEmaMinSlopePointsPerBar;
                    ...
                    if (v2PrepareConditionPass)
                    {
                        ...
                        LogDebug(string.Format(
                            "V2 prepared | bar={0} adx={1:0.00} emaSlope={2:0.000} dir={3}",
                            CurrentBar,
                            v2EntryAdxAtSignal,
                            v2EmaSlopeAtSignal,
                            v2PreparedDirection));
                ...
                bool readyAfterSlope = false;
                if (v2Prepared)
                {
                    readyAfterSlope = v2PreparedDirection == SessionTradeDirection.LongOnly
                        ? emaSlopeLongPass
                        : emaSlopeShortPass;
                }
                ...
                // 2) EMA min slope pass (latched)
                // 3) ADX momentum pass (latched)
                // 4) First qualifying close beyond EMA triggers entry
                if (v2Prepared && !v2StepEmaSlopePassed && readyAfterSlope)
                {
                    v2StepEmaSlopePassed = true;
                    LogDebug(string.Format(
                        "V2 step passed | step=EmaSlope dir={0} slope={1:0.000}",
                        v2PreparedDirection,
                        GetEmaSlopePointsPerBar()));
                }

                if (v2Prepared && v2StepEmaSlopePassed && !v2StepAdxMomentumPassed && adxSlopePass)
                {
                ...
                // are allowed even if ADX rises above min, as long as momentum gate passes.
                bool v2PreEntryAdxGatePass = adxSlopePass;
                if (v2Prepared && v2StepEmaSlopePassed && v2StepAdxMomentumPassed && canTradeNowNoAdx && v2PreEntryAdxGatePass)
                {
                    if (v2PreparedDirection == SessionTradeDirection.LongOnly)
                        longSignalRaw = bullish && bodyAbovePercent > 0.0;
                    else
                        shortSignalRaw = bearish && bodyBelowPercent > 0.0;
                }
                ...
                if (DebugLogging && v2Prepared && !longSignalRaw && !shortSignalRaw)
                {
                    ...
                    {
                        if (!inActiveSessionNow)
                            v2WaitReasons.Add("out-of-session");
                        if (inNewsSkipNow)
                            v2WaitReasons.Add("news-block");
                        if (inNySkipNow)
                            v2WaitReasons.Add("ny-skip");
                        if (isAsiaSundayBlockedNow)
                            v2WaitReasons.Add("asia-sunday-block");
                        if (accountBlocked)
                            v2WaitReasons.Add("account-blocked");
                        if (!maxTradesPass)
                            v2WaitReasons.Add(string.Format("max-trades {0}/{1}", tradesThisSession, MaxTradesPerSession));
                    }
                    if (!v2StepEmaSlopePassed)
                    {
                        if (v2PreparedDirection == SessionTradeDirection.LongOnly)
                    ...
                    else if (!v2StepAdxMomentumPassed)
                    ...
                    if (v2PreparedDirection == SessionTradeDirection.LongOnly)
                    {
                        ...
                        if (!(bodyAbovePercent > 0.0))
                            v2WaitReasons.Add(string.Format("candle-not-close-above-ema above%={0:0.0}", bodyAbovePercent));
                    }
                    else if (v2PreparedDirection == SessionTradeDirection.ShortOnly)
                    {
                        if (!bearish)
                            v2WaitReasons.Add(string.Format("candle-not-bearish o={0:0.00} c={1:0.00}", Open[0], Close[0]));
                        if (!(bodyBelowPercent > 0.0))
                            v2WaitReasons.Add(string.Format("candle-not-close-below-ema below%={0:0.0}", bodyBelowPercent));
                    }

                    if (v2WaitReasons.Count == 0)
                    ...
                    if (v2WaitChanged || v2WaitHeartbeat)
                    {
                        LogDebug(string.Format(
                            ...
                            emaValue,
                            reasonsText));
                        v2LastWaitSignature = v2WaitSignature;
                        v2LastWaitLogBar = CurrentBar;
                    }
                }

                // Defensive timeout to avoid stale prepared state across long inactive periods.
                if (v2Prepared && CurrentBar - v2PreparedBarIndex > 200)
                {
                    v2Prepared = false;
                    v2PreparedDirection = SessionTradeDirection.Both;
                    v2PreparedBarIndex = -1;
            ...
            {
                bool v1AdxAtOrBelowMinPass = !inActiveSessionNow || activeAdxThreshold <= 0.0 || adxValue <= activeAdxThreshold;
                longSignalRaw = v1AdxAtOrBelowMinPass && emaSlopeLongPass && bullish && bodyAbovePercent > 0.0;
                shortSignalRaw = v1AdxAtOrBelowMinPass && emaSlopeShortPass && bearish && bodyBelowPercent > 0.0;
            }
            bool longSignal = longSignalRaw && allowLong;
            bool shortSignal = shortSignalRaw && allowShort;
            bool canTradeEntriesNow = canTradeNowNoAdx;

            bool v1UseDynamicExitArming = EntryModel == EntryModelMode.EntryModelV1 && ArmDynamicExitsAfterAdxRiseV1 > 0.0;
            if (Position.MarketPosition != MarketPosition.Flat && v1UseDynamicExitArming && !v1DynamicExitsArmed
                && adxValue >= v1EntryAdxAtFill + ArmDynamicExitsAfterAdxRiseV1)
            {
                v1DynamicExitsArmed = true;
                LogDebug(string.Format(
                    "V1 exits armed | adx={0:0.00} entryAdx={1:0.00} threshold={2:0.00}",
                    adxValue,
                    v1EntryAdxAtFill,
                    ArmDynamicExitsAfterAdxRiseV1));
            }
            bool v2UseDynamicExitArming = EntryModel == EntryModelMode.EntryModelV2 && ArmDynamicExitsAfterAdxRiseV1 > 0.0;
            if (Position.MarketPosition != MarketPosition.Flat && v2UseDynamicExitArming && !v2DynamicExitsArmed
                && adxValue >= v2EntryAdxAtFill + ArmDynamicExitsAfterAdxRiseV1)
            {
                v2DynamicExitsArmed = true;
                LogDebug(string.Format(
                    "V2 exits armed | adx={0:0.00} entryAdx={1:0.00} threshold={2:0.00}",
                    adxValue,
                    v2EntryAdxAtFill,
                    ArmDynamicExitsAfterAdxRiseV1));
            }

            bool dynamicExitsAllowed = true;
            if (EntryModel == EntryModelMode.EntryModelV1)
                dynamicExitsAllowed = v1DynamicExitsArmed;
            else if (EntryModel == EntryModelMode.EntryModelV2)
                dynamicExitsAllowed = v2DynamicExitsArmed;

            if (Position.MarketPosition == MarketPosition.Long)
            {
                if (dynamicExitsAllowed && activeAdxAbsoluteExitLevel > 0.0 && adxValue >= activeAdxAbsoluteExitLevel)
                {
                ...
                double adxDrawdown;
                if (dynamicExitsAllowed && ShouldExitOnAdxDrawdown(adxValue, out adxDrawdown))
                {
                ...
                if (dynamicExitsAllowed && Close[0] <= emaValue - activeExitCrossPoints)
                {
            ...
            MarketPosition marketPosition, string orderId, DateTime time)
        {
            ...
            if (IsEntryOrderName(orderName))
            {
                ...
                if (EntryModel == EntryModelMode.EntryModelV1)
                {
                    v1EntryAdxAtFill = activeAdx != null ? activeAdx[0] : 0.0;
                    v1DynamicExitsArmed = ArmDynamicExitsAfterAdxRiseV1 <= 0.0;
                }
                else if (EntryModel == EntryModelMode.EntryModelV2)
                {
                    v2EntryAdxAtFill = activeAdx != null ? activeAdx[0] : 0.0;
                    v2DynamicExitsArmed = ArmDynamicExitsAfterAdxRiseV1 <= 0.0;
                }
                v2Prepared = false;
                v2PreparedDirection = SessionTradeDirection.Both;
                v2PreparedBarIndex = -1;
        ...
        [NinjaScriptProperty]
        [Display(Name = "Block Sunday Trades", Description = "If enabled, block new Asia entries on Sundays.", GroupName = "Asia", Order = 3)]
        ...
        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "Exit Cross Points", Description = "Additional points beyond EMA before evaluating exit. 0 means EMA touch/cross.", GroupName = "Asia", Order = 17)]
        ...
        [NinjaScriptProperty]
        [Range(0.0, double.MaxValue)]
        [Display(Name = "Exit Cross Points", Description = "Additional points beyond EMA before evaluating exit. 0 means EMA touch/cross.", GroupName = "New York", Order = 17)]
        ...
        [NinjaScriptProperty]
        [Display(Name = "Entry Confirmation", Description = "Show a Yes/No confirmation popup before each new long/short entry.", GroupName = "13. Risk", Order = 5)]
        public bool RequireEntryConfirmation { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Entry Model", Description = "Choose Entry Model V1 or Entry Model V2.", GroupName = "13. Risk", Order = 8)]
        public EntryModelMode EntryModel { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, 100.0)]
        [Display(Name = "Arm Dynamic Exits After ADX Rise (V1)", Description = "0 disables. When using Entry Model V1/V2, dynamic exits (ADX drawdown, ADX absolute, EMA cross exits) are disabled until ADX >= entryADX + this value.", GroupName = "13. Risk", Order = 9)]
        public double ArmDynamicExitsAfterAdxRiseV1 { get; set; }

        [NinjaScriptProperty]
        [Range(0, 20)]
        [Display(Name = "V2: Swing candles before", Description = "Swing validation window (before). 0 disables.", GroupName = "13. Risk", Order = 10)]
        public int EntryModelV2_SwingCandlesBefore { get; set; }

        [NinjaScriptProperty]
        [Range(0, 20)]
        [Display(Name = "V2: Swing candles after", Description = "Swing validation window (after). 0 disables.", GroupName = "13. Risk", Order = 11)]
